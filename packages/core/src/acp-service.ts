...
import type { ACPConfigOption } from "@dotagents/shared"
...
/**
 * Set the ProgressEmitter for the ACP service.
 */
export function setACPServiceProgressEmitter(emitter: ProgressEmitter): void {
...
class ACPService extends EventEmitter {
  ...
  getAgents(): Array<{ config: ACPAgentConfig; status: ACPAgentStatus; error?: string }> {
    ...
    return acpAgents.map((agentConfig: ACPAgentConfig) => {
  ...
  async spawnAgent(agentName: string, options: ACPSpawnOptions = {}): Promise<ACPSpawnResult> {
    ...
    if (profile && (profile.connection.type === "acp" || profile.connection.type === "stdio")) {
      ...
      agentConfig = config.acpAgents?.find((a: ACPAgentConfig) => a.name === agentName)
    }
  ...
  async runTask(request: ACPRunRequest): Promise<ACPRunResponse> {
    ...
    try {
      ...
      const contentBlocks = promptResult?.content
        ...
        || ((promptResult as Record<string, unknown>)?.messages as Array<Record<string, unknown>> | undefined)?.[0]?.content
        || []
