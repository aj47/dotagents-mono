...
describe("settings general observability defaults", () => {
  it("uses shared observability defaults", () => {
    ...
    expect(source).toContain("LANGFUSE_CREDENTIAL_FIELD_METADATA")
    expect(source).toContain("LOCAL_TRACE_LOGGING_SETTING_METADATA")
    expect(source).toContain("type LangfuseDraftKey")
    expect(source).not.toContain("function getLangfuseDrafts")
    expect(source).not.toContain('label="Local trace logging"')
    expect(source).not.toContain('placeholder="pk-lf-..."')
    expect(source).not.toContain('placeholder="sk-lf-..."')
  })
})
