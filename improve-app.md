...
- 2026-03-08: Mobile `SessionListScreen` in `apps/mobile/src/screens/SessionListScreen.tsx` likely shares the same raw-title readability risk because the list still renders `item.title` directly, but this pass intentionally stayed desktop-scoped because mobile uses a separate local session model and needs its own focused investigation before changing copy or fallback rules.
...
- Mobile `SessionListScreen` likely shares the just-confirmed blank-title readability risk because it still renders `item.title` directly, so the next contained product-quality pass is to verify whether empty local session titles can reach that list and, if so, apply the same kind of readable fallback without over-broad session-store refactors.
