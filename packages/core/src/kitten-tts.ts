 ...
 */
export function setKittenTTSPathResolver(resolver: PathResolver): void {
  pathResolver = resolver
}

function requirePathResolver(): PathResolver {
  if (!pathResolver) {
    throw new Error("[Kitten] PathResolver not set. Call setKittenTTSPathResolver() before using TTS.")
  }
  return pathResolver
}
...
function getModelsPath(): string {
  return path.join(requirePathResolver().getUserDataPath(), "models", "kitten")
}
