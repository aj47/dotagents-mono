...
export async function runInternalSubSession(
  ...
  try {
    ...
    const result = await agentRuntime.runAgentTurn({
      ...
      conversationId: subSession.conversationId, // Use a sub-session-specific conversation when available
      sessionId: subSessionId,
...
export async function continueInternalSubSession(
  ...
  try {
    ...
    const cfg = configStore.get();
    const effectiveSubSessionMaxIterations = resolveAgentModeMaxIterations(cfg, options.maxIterations);
