...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(screenSource, /contentSize\.height - bottomResumeThresholdPx/);
  assert.match(screenSource, /contentOffset\.y <= topLoadThresholdPx/);
  assert.match(screenSource, /if \(isNearTop && visibleMessageCount < messages\.length\) \{\s+loadEarlierMessages\(\);/);
