...
test('lets mobile respond to desktop tool approval requests from progress updates', () => {
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationThreadBodyMobileState\(\{[\s\S]*?inlineActivity: getChatRuntimeInlineActivityMobileIndicatorState\(\{/);
  assert.match(sessionPresentationSource, /toolApproval: getChatRuntimeConversationToolApprovalMobileState\(\{\s+message,\s+expandedToolApprovals,\s+pendingApprovalResponseId,\s+colors,\s+onToggleArguments: onToggleToolApprovalArguments,\s+onRespondToToolApproval,/);
...
test('shows desktop-style retry status updates from shared runtime presentation', () => {
  ...
  assert.match(sessionPresentationSource, /retryStatus: getChatRuntimeConversationRetryStatusMobileState\(\{\s+message,\s+colors,\s+\}\),/);
...
test('renders delegated agent progress as compact desktop-style mobile chrome', () => {
  ...
  assert.match(screenSource, /threadList: \{[\s\S]*?messages,[\s\S]*?visibleMessageCount,[\s\S]*?groupByIndex: toolActivityGroups\.groupByIndex,/);
  assert.match(sessionPresentationSource, /delegationCard: getChatRuntimeConversationDelegationCardMobileState\(\{\s+message,\s+surface: presentation\.delegationSurface,\s+toolEntries: renderedToolEntries,/);
...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(chatMessageChromeSource, /getChatRuntimeInlineActivityMobileIndicatorState,/);
  ...
  assert.match(screenSource, /threadList: \{\s+messages,\s+visibleMessageCount,\s+groupByIndex: toolActivityGroups\.groupByIndex,\s+groupState: expandedGroups,\s+inheritedState: expandedMessages,\s+onToggleGroup: toggleGroupExpansion,/);
  assert.match(chatMessageChromeSource, /inlineActivity: getChatRuntimeInlineActivityMobileIndicatorState\(\{\s+message,\s+isResponding,\s+spinnerSource,\s+\}\),/);
...
test('uses desktop-style streaming response chrome while mobile assistant content is live', () => {
  ...
  assert.match(sessionPresentationSource, /onPress: canToggleCollapsedPreview\s+\? \(\) => onToggleMessageExpansion\(messageIndex\)\s+: undefined,/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(sessionPresentationSource, /actionSet: getChatRuntimeConversationActionSetMobileState\(\{\s+message,\s+messageIndex,\s+visibleMessageContent,\s+turnDuration,/);
  ...
  assert.match(chatMessageChromeSource, /const actionRenderState = getChatRuntimeConversationMessageActionsMobileRenderState\(\{\s+message: messageRenderState,[\s\S]*?copy: \{\s+role: copy\.role,\s+content: copy\.content,\s+isAssistantComplete: copy\.isAssistantComplete,\s+isCopied: copy\.isCopied,\s+colors: copy\.colors,\s+\},\s+\}\);/);
...
test('uses shared desktop chat message presentation tones for mobile message cards', () => {
  ...
  assert.match(sessionPresentationSource, /collapsed: \{\s+onPress: canToggleCollapsedPreview\s+\? \(\) => onToggleMessageExpansion\(messageIndex\)\s+: undefined,\s+\},/);
  assert.match(chatMessageChromeSource, /collapsed: createChatMessageCollapsedPreviewProps\(\{\s+renderState: messageRenderState\.collapsedPreview,\s+actionState: messageRenderState\.collapsedPreviewAction,\s+\.\.\.collapsed,\s+\}\),/);
...
test('lets mobile branch linked desktop conversations from individual messages', () => {
  ...
  assert.match(chatMessageChromeSource, /branch: \{\s+conversationId,\s+role: branchRole,\s+branchMessageIndex,\s+fallbackMessageIndex,\s+pendingMessageIndex,\s+colors: branchColors,\s+\},/);
  assert.match(sessionPresentationSource, /const branchRenderState = getChatRuntimeBranchMobileRenderState\(branch\)/);
  assert.match(chatMessageChromeSource, /if \(actionRenderState\.branch\.messageIndex != null\) \{\s+onBranchMessage\?\.\(actionRenderState\.branch\.messageIndex\);/);
  assert.match(chatMessageChromeSource, /accessibilityRole=\{spec\.renderState\.accessibilityRole\}/);
  assert.match(chatMessageChromeSource, /accessibilityLabel=\{spec\.renderState\.accessibilityLabel\}/);
  assert.match(chatMessageChromeSource, /accessibilityState=\{spec\.renderState\.accessibilityState\}/);
