 ...
 *
 * Extracted to @dotagents/core — external dependencies (runAgentLoopSession,
 * agentSessionTracker) are injected via setters.
 */

import { configStore, globalAgentsFolder, resolveWorkspaceAgentsFolder } from "./config"
import { logApp } from "./debug"
// ConversationService is injected via setLoopConversationService
import { agentProfileService, createSessionSnapshotFromProfile } from "./agent-profile-service"
import type { LoopConfig, SessionProfileSnapshot } from "./types"
...
// ============================================================================
// Injectable dependencies — set by the host app
// ============================================================================

/** Function to run an agent loop session — injected by host app (e.g., from tipc) */
let _runAgentLoopSession: ((prompt: string, conversationId: string, sessionId: string) => Promise<void>) | null = null

/** Agent session tracker — injected by host app */
let _agentSessionTracker: {
  startSession(conversationId: string, title: string, isLoop: boolean, snapshot?: SessionProfileSnapshot): string
} | null = null

/** Conversation service instance — injected or imported */
let _conversationService: {
  createConversation(content: string, role: string): Promise<{ id: string }>
} | null = null

/**
 * Set the loop runner function.
 * Must be called before starting any loops.
 */
export function setLoopRunner(fn: (prompt: string, conversationId: string, sessionId: string) => Promise<void>): void {
  _runAgentLoopSession = fn
}

/**
 * Set the agent session tracker.
 */
export function setLoopSessionTracker(tracker: {
  startSession(conversationId: string, title: string, isLoop: boolean, snapshot?: SessionProfileSnapshot): string
}): void {
...
/**
 * Set the conversation service for loop execution.
 */
export function setLoopConversationService(service: {
  createConversation(content: string, role: string): Promise<{ id: string }>
}): void {
  ...
  private async executeLoop(loopId: string, options: { rescheduleAfterRun: boolean }): Promise<void> {
    ...
    try {
      ...
      if (!_conversationService) {
        logApp("[LoopService] Cannot execute loop: conversation service not set")
        return
      }
      if (!_agentSessionTracker) {
        logApp("[LoopService] Cannot execute loop: session tracker not set")
        return
      }
      if (!_runAgentLoopSession) {
        logApp("[LoopService] Cannot execute loop: loop runner not set")
        return
      }

      const conversation = await _conversationService.createConversation(loop.prompt, "user")
      const conversationTitle = `[Repeat] ${loop.name}`
      const sessionId = _agentSessionTracker.startSession(
        conversation.id,
        conversationTitle,
        true,
        profileSnapshot
      ...
      // Reuse the main agent execution flow via the injected runner.
      await _runAgentLoopSession(loop.prompt, conversation.id, sessionId)
    } catch (error) {
