...
test('desktop before-quit cleanup waits for ACP, MCP, and remote server shutdown together', () => {
  assert.doesNotMatch(beforeQuitSection, /acpService\.shutdown\(\)\.catch\(/)
  assert.match(beforeQuitSection, /const cleanupTasks = \[\s*\{ label: "ACP service shutdown", run: \(\) => acpService\.shutdown\(\) \},\s*\{ label: "MCP service cleanup", run: \(\) => mcpService\.cleanup\(\) \},\s*\{ label: "remote server shutdown", run: \(\) => stopRemoteServer\(\) \},\s*\] as const/)
