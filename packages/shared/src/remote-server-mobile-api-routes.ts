...
export interface RemoteServerMobileApiRequestLike {
  body: unknown;
  headers: {
...
export interface RemoteServerMobileApiReplyLike {
  code(statusCode: number): { send(body?: unknown): unknown };
  header(name: string, value: string): unknown;
}

export type RemoteServerMobileApiRouteResult = unknown | Promise<unknown>;

export type RemoteServerMobileApiRouteHandler<
  Request extends RemoteServerMobileApiRequestLike,
  Reply extends RemoteServerMobileApiReplyLike,
> = (request: Request, reply: Reply) => RemoteServerMobileApiRouteResult;

export interface RemoteServerMobileApiRouteServer<
  Request extends RemoteServerMobileApiRequestLike = RemoteServerMobileApiRequestLike,
  Reply extends RemoteServerMobileApiReplyLike = RemoteServerMobileApiReplyLike,
> {
  post(path: string, handler: RemoteServerMobileApiRouteHandler<Request, Reply>): unknown;
  get(path: string, handler: RemoteServerMobileApiRouteHandler<Request, Reply>): unknown;
  patch(path: string, handler: RemoteServerMobileApiRouteHandler<Request, Reply>): unknown;
  put(path: string, handler: RemoteServerMobileApiRouteHandler<Request, Reply>): unknown;
  delete(path: string, handler: RemoteServerMobileApiRouteHandler<Request, Reply>): unknown;
}
...
export function registerMobileApiRoutes<
  ...
  Reply extends RemoteServerMobileApiReplyLike = RemoteServerMobileApiReplyLike,
>(
  fastify: RemoteServerMobileApiRouteServer<Request, Reply>,
  options: MobileApiRouteOptions<Request, Reply>,
): void {
  const {
    actions,
    providerSecretMask,
