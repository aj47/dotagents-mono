...
describe("remote-server route registration", () => {
  ...
  it("preserves task-specific max-iteration caps in repeat task routes", () => {
    ...
    expect(updateLoopSection).toContain("body.maxIterations !== null")
    expect(updateLoopSection).toContain("body.maxIterations === null")
    expect(updateLoopSection).toContain("...(body.maxIterations !== undefined && { maxIterations: maxIterations ?? undefined })")
  })
})
