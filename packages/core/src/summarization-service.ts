...
import type { AgentStepSummary, ModelPreset } from '@dotagents/shared'
...
function getPresetById(presetId: string): ModelPreset | undefined {
  ...
  const allPresets = builtIn.map(preset => {
    ...
    return saved ? { ...preset, ...Object.fromEntries(Object.entries(saved).filter(([_, v]: [string, unknown]) => v !== undefined)) } : preset
  })
