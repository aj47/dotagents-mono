 ...
 * Skills Service
 *
 * Manages agent skills (CRUD, GitHub import, file watching).
 * Extracted to @dotagents/core — uses PathResolver and ProgressEmitter
 * instead of Electron's app.getPath and WINDOWS/tipc.
 */
...
function getSkillsPath(): string {
  ...
  return path.join(pathResolver.getAppDataPath(), process.env.APP_ID || "dotagents", "skills.json")
}
...
function getSkillsFolder(): string {
  const pathResolver = container.resolve<PathResolver>(ServiceTokens.PathResolver)
  return path.join(pathResolver.getAppDataPath(), process.env.APP_ID || "dotagents", "skills")
}

/**
 * Bundled skills path resolver — injected by host app.
 * Desktop sets this to point to the app's resources/bundled-skills directory.
 * CLI may set it to a different location or leave it unset.
 */
let _bundledSkillsPath: string | null = null

/**
 * Set the path to bundled skills (shipped with the app).
 * Should be called by the host app at startup.
 */
export function setBundledSkillsPath(bundledPath: string): void {
  _bundledSkillsPath = bundledPath
}

function getBundledSkillsPath(): string {
  if (_bundledSkillsPath) {
    return _bundledSkillsPath
  }
  // Fallback: use a path relative to the current working directory
  return path.join(process.cwd(), "resources", "bundled-skills")
}
...
class SkillsService {
  ...
  private migrateLegacySkillsJsonToAgents(globalLayer: AgentsLayerPaths, existingSkillIds: Set<string>): number {
    if (!fs.existsSync(getSkillsPath())) return 0

    let data: AgentSkillsData | null = null
    try {
      data = JSON.parse(fs.readFileSync(getSkillsPath(), "utf8")) as AgentSkillsData
    } catch (error) {
    ...
    try {
      const migratedPath = getSkillsPath() + ".migrated"
      fs.renameSync(getSkillsPath(), migratedPath)
...
`
  ...
  async importSkillFromGitHub(repoIdentifier: string): Promise<{
    ...
    const cloneDir = path.join(getSkillsFolder(), `${owner}--${repo}`)
    ...
    try {
      if (fs.existsSync(cloneDir)) {
        ...
        fs.mkdirSync(getSkillsFolder(), { recursive: true })
...
/**
 * Notify all UI surfaces that the skills folder has changed.
 * Uses ProgressEmitter to abstract away the notification mechanism.
 */
function notifySkillsFolderChanged(): void {
  try {
    const progressEmitter = container.tryResolve<ProgressEmitter>(ServiceTokens.ProgressEmitter)
    if (progressEmitter) {
      progressEmitter.emitEvent("skillsFolderChanged", {})
    }
  } catch {
    // No emitter registered yet, ignore
  }
}
