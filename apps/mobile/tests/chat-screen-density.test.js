...
test('uses shared runtime presentation for the mobile chat viewport and loading state', () => {
  ...
  assert.match(screenSource, /const chatMessageRuntimeSurface = createChatMessageRuntimeChromeProps<PredefinedPromptSummary, Loop>\(\{\s+spinnerSource: chatRuntimeSpinnerSource,\s+composerChrome: \{\s+colors: theme\.colors,\s+platform: Platform\.OS,\s+\},/);
  ...
  assert.match(chatMessageChromeSource, /const chatMessageRuntimeViewport = createChatMessageRuntimeViewportChromeProps\(\{\s+\.\.\.viewport,\s+loadingSpinnerSource: spinnerSource,\s+visibleMessageCount: conversationThreadListState\.visibleMessageCount,/);
  ...
  assert.match(screenSource, /const chatMessageRuntimeSurface = createChatMessageRuntimeChromeProps<PredefinedPromptSummary, Loop>\(\{\s+spinnerSource: chatRuntimeSpinnerSource,/);
  ...
  assert.match(chatMessageChromeSource, /const chatMessageRuntimeViewport = createChatMessageRuntimeViewportChromeProps\(\{\s+\.\.\.viewport,\s+loadingSpinnerSource: spinnerSource,/);
  assert.doesNotMatch(screenSource, /loadingSpinnerSource: isDark \? darkSpinner : lightSpinner,/);
...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(chatMessageChromeSource, /const conversationThreadListState = createChatMessageConversationRuntimeThreadListRenderState\(\{\s+\.\.\.threadListInput,\s+spinnerSource,\s+\}\);/);
...
test('suppresses duplicate auto TTS starts for the same mobile response text', () => {
  ...
  assert.match(screenSource, /useChatMessageRuntimeAssistantSpeechActionsState,/);
  assert.match(screenSource, /const \{ speakAssistantResponse \} = useChatMessageRuntimeAssistantSpeechActionsState\(\{\s+ttsEnabledRef,\s+recentAutoSpeechByTextRef,\s+config,\s+effectiveTtsProvider,\s+effectiveRemoteTtsVoice,\s+effectiveRemoteTtsModel,\s+effectiveRemoteTtsRate,\s+handsFree,\s+handsFreeController,\s+speakNative: Speech\.speak,\s+speakRemote: speakRemoteTts,\s+voiceLog,\s+\}\);/);
...
test('replaces the empty mobile chat home state with quick-start launchers', () => {
  ...
  assert.match(screenSource, /useChatConversationHomePromptTaskRunActionsState,/);
  assert.match(screenSource, /const \{ handleRunPromptTask \} = useChatConversationHomePromptTaskRunActionsState<Loop, ExtendedSettingsApiClient>\(\{\s+taskClient: settingsClient,\s+canRunPromptTask,\s+beginPromptTaskRun,\s+clearPromptTaskRun,\s+showAlert: Alert\.alert,\s+\}\);/);
