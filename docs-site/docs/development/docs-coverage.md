...
|-------------|--------------|--------------|-------|
| `apps/desktop` | Electron app package, builds, release scripts, desktop runtime | [Desktop App](/desktop/overview), [Architecture Deep Dive](architecture), [Development Setup](setup) | The desktop app is Electron-first; renderer validation alone does not cover desktop behavior. |
...
| `packages/core` | Platform-agnostic config, `.agents` file parsing, service container, state primitives, test adapters | [Architecture Deep Dive](architecture), [.agents Protocol](/concepts/dot-agents-protocol) | Keep this package dependency-light and usable outside Electron. |
| `packages/shared` | Cross-app API contracts, progress/session/message types, providers, colors, language/STT/TTS utilities | [Architecture Deep Dive](architecture), [Remote API Reference](/reference/api) | If mobile and desktop both import it, document the shape here or in the feature doc that owns it. |
...
| `docs-site` | Docusaurus documentation app | [Development Setup](setup), this page | Build with `pnpm --dir docs-site build`; keep sidebar generated by categories. |
| `website` | Static marketing site and deploy assets for dotagents.app | Root README, website README, this page | The docs site covers product behavior; website docs cover static asset/deploy mechanics. |
| `apps/promo-studio` | Rendered marketing/demo clips used by the website and launch materials | This page | Currently tracked as rendered media only. Add a package README if source projects/scripts are added. |
| `scripts` | Root installer and release support scripts | [Development Setup](setup), this page | Use these docs for source install, build, and verification commands. |
