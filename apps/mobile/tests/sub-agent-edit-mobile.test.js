...
test('LoopEditScreen keeps profile load failures local to the Agent Profile section', () => {
  ...
  assert.match(loopEditSource, /showProfileLoadErrorNotice && \([\s\S]*?style=\{\[styles\.profileNoticeContainer, styles\.profileNoticeWarningContainer\]\}[\s\S]*?\{profileLoadErrorNoticeText\}[\s\S]*?style=\{styles\.profileNoticeRetryButton\}[\s\S]*?onPress=\{handleRetryProfiles\}[\s\S]*?createButtonAccessibilityLabel\('Retry loading saved profiles'\)[\s\S]*?Attempts to load saved profiles for this loop again\.[\s\S]*?Retry Saved Profiles[\s\S]*?\)/);
  ...
  assert.match(loopEditSource, /profileNoticeRetryButton:\s*\{[\s\S]*?\.\.\.noticeActionTouchTarget,[\s\S]*?alignSelf:\s*'stretch',[\s\S]*?borderColor:\s*theme\.colors\.primary \+ '26',[\s\S]*?alignItems:\s*'center',[\s\S]*?justifyContent:\s*'center'/);
  assert.match(loopEditSource, /profileNoticeRetryButtonText:\s*\{[\s\S]*?color:\s*theme\.colors\.primary,[\s\S]*?textAlign:\s*'center'/);
