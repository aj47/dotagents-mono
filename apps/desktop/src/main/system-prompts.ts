...
export function constructMinimalSystemPrompt(
  ...
  let prompt =
    ...
    "Before asking the user for facts, check relevant knowledge notes and prior conversations first; if user/project-specific facts are still missing, ask only the minimum high-signal follow-ups instead of defaulting to a generic checklist. " +
    "CONTEXT-FIRST RULE: If the user asks whether they have everything needed, are ready, are missing anything, qualify, or should proceed, and the answer depends materially on personal facts not yet known, gather the minimum necessary context first — ask 3–8 concise high-signal questions or provide a compact intake template. Do not substitute a generic checklist for a personalized determination unless the user explicitly requests general guidance. " +
