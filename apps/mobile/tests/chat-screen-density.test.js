...
test('resolves mobile monospace typography from shared surface tokens', () => {
  ...
  assert.match(screenSource, /toolApprovalArgumentsPreview:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(toolApprovalSurface\.argumentsPreview\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolApprovalArgumentsFull:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(toolApprovalSurface\.fullArguments\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolCallCompactName:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(compactToolExecution\.name\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolActivityGroupCountBadgeText:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(toolActivityGroupSurface\.countBadge\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolActivityGroupPreviewLine:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(toolActivityGroupSurface\.preview\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolName:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.toolName\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolPayloadPreview:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.payloadPreview\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolParamsCode:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.codeBlock\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolResultCharCount:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.characterCount\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolResultCode:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.codeBlock\.fontFamilyByPlatform\)/);
  assert.match(screenSource, /toolResultErrorText:\s*\{[\s\S]*?fontFamily:\s*resolveMobileFontFamily\(detailedToolExecution\.codeBlock\.fontFamilyByPlatform\)/);
...
test('lets mobile respond to desktop tool approval requests from progress updates', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationRuntimeThreadListRenderState\(\{[\s\S]*?expandedToolApprovals,[\s\S]*?pendingApprovalResponseId: pendingToolApprovalResponseId,[\s\S]*?onToggleToolApprovalArguments: toggleToolApprovalArguments,[\s\S]*?onRespondToToolApproval: respondToToolApproval,/);
...
test('uses shared runtime presentation for the mobile chat viewport and loading state', () => {
  ...
  assert.match(screenSource, /threadStates: conversationThreadStates,\s+threadStyles: chatMessageConversationThreadStyles\.runtimeThread,/);
  ...
  assert.match(screenSource, /useRef<ChatMessageScrollViewportRef>\(null\)/);
  assert.match(screenSource, /const handleScroll = useCallback\(\(event: ChatMessageScrollEvent\) => \{/);
  ...
  assert.match(screenSource, /visibleMessageCount: conversationThreadVisibleMessageCount,\s+totalMessageCount: conversationThreadTotalMessageCount,\s+hiddenMessageCount,\s+messageHistoryLoadIncrement: CHAT_MESSAGE_HISTORY_WINDOW\.loadIncrement,\s+latestStepSummary,\s+colors: theme\.colors,\s+onLoadEarlierMessages: handleLoadEarlierMessages,/);
  ...
  assert.match(screenSource, /const chatMessageRuntimeDock = createChatMessageRuntimeDockChromeProps\(\{[\s\S]*?responseHistoryResponses: respondToUserHistory,[\s\S]*?scrollToBottomVisible: !shouldAutoScroll,[\s\S]*?voiceOverlayListening: listening,[\s\S]*?queuePanelEnabled: messageQueueEnabled,[\s\S]*?connectionState,[\s\S]*?lastFailedMessage,[\s\S]*?isResponding: responding,[\s\S]*?colors: theme\.colors,[\s\S]*?composer: chatComposerRuntimeDock,/);
...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(screenSource, /visibleMessageCount: conversationThreadVisibleMessageCount,\s+totalMessageCount: conversationThreadTotalMessageCount,\s+hiddenMessageCount,\s+messageHistoryLoadIncrement: CHAT_MESSAGE_HISTORY_WINDOW\.loadIncrement,\s+latestStepSummary,\s+colors: theme\.colors,/);
  ...
  assert.match(screenSource, /onLoadEarlierMessages: handleLoadEarlierMessages,/);
  assert.match(chatMessageChromeSource, /<ChatMessageHistoryBanner\s+\{\.\.\.historyBanner\}\s+styles=\{styles\.historyBanner\}/);
...
test('uses desktop-style streaming response chrome while mobile assistant content is live', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationRuntimeThreadListRenderState\(\{[\s\S]*?messages,[\s\S]*?visibleMessageCount,[\s\S]*?spinnerSource: isDark \? darkSpinner : lightSpinner,[\s\S]*?onToggleMessageExpansion: toggleMessageExpansion,/);
...
test('surfaces desktop step summaries as compact mobile runtime chrome without persisting them as messages', () => {
  ...
  assert.match(screenSource, /latestStepSummary,\s+colors: theme\.colors,\s+onLoadEarlierMessages: handleLoadEarlierMessages,/);
  assert.match(chatMessageChromeSource, /<ChatMessageStepSummaryCard\s+\{\.\.\.stepSummary\}\s+styles=\{styles\.stepSummary\}/);
...
test('uses tool activities wording consistently for grouped tool activity labels', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationRuntimeThreadListRenderState,/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationRuntimeThreadListRenderState\(\{[\s\S]*?messages,[\s\S]*?visibleMessageCount,[\s\S]*?turnDurationsByUserTimestamp: turnDurations\.byUserTimestamp,/);
  ...
  assert.match(screenSource, /threadStates: conversationThreadStates,/);
...
test('shows shared per-turn duration badges on mobile user messages', () => {
  ...
  assert.match(screenSource, /createChatMessageRuntimeTurnDurationMessages,/);
  ...
  assert.match(screenSource, /const id = setInterval\(\(\) => setTurnNow\(Date\.now\(\)\), 1000\);/);
  assert.match(screenSource, /const turnDurationMessages = useMemo\(\s+\(\) => createChatMessageRuntimeTurnDurationMessages\(messages\),\s+\[messages\],\s+\);/);
...
test('lets mobile edit and delete desktop saved prompts from quick-start cards', () => {
  ...
  assert.match(screenSource, /const chatMessageRuntimeSurface = createChatMessageRuntimeSurfaceChromeProps\(\{[\s\S]*?promptEditorVisible: addPromptModalVisible,/);
