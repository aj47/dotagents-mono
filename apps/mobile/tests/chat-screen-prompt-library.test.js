...
test('shows desktop library items directly in the new-chat prompt launchers', () => {
  ...
  assert.match(chatMessageChromeSource, /type PromptLibraryEditorMobileChromeState,/);
  assert.match(chatMessageChromeSource, /type PromptLibraryMobileShortcutChromeState,/);
...
test('can create a new predefined prompt from mobile and save it to desktop settings', () => {
  ...
  assert.match(screenSource, /const promptEditorModalChrome = useMemo\(\s+\(\) => createChatConversationHomePromptEditorModalChromeProps\(\{\s+surface: promptEditorModalSurface,\s+colors: promptLibrarySurfaceColors\.editorModal,[\s\S]*?editorChrome: promptLibraryEditorChrome,/);
