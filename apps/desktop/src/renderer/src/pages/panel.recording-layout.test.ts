...
describe("panel recording layout", () => {
  ...
  it("lets MCP submit routes own conversation history so panel input does not duplicate user messages", () => {
    ...
    expect(compactTipcSource).toContain(compact("const conversation = await conversationService.createConversation("))
    expect(compactTipcSource).toContain(compact("input.text,"))
    expect(compactTipcSource).toContain(compact("await conversationService.addMessageToConversation("))
    expect(compactTipcSource).toContain(compact("conversationId,"))
    expect(compactTipcSource).toContain(compact("transcript,"))
