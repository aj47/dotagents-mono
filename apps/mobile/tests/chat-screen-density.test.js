...
test('renders delegated agent progress as compact desktop-style mobile chrome', () => {
  ...
  assert.match(chatMessageChromeSource, /export function ChatMessageDelegationToolPreviewRow[\s\S]*?statusIcon\.spinner\.shouldRender \? \([\s\S]*?size=\{statusIcon\.spinner\.size\}[\s\S]*?color=\{statusIcon\.spinner\.color\}[\s\S]*?export function ChatMessageDelegationMorePreviewAction/);
  assert.match(sessionPresentationSource, /renderState\.statusIndicator\.icon\.shouldRender/);
  assert.match(chatMessageChromeSource, /export function ChatMessageDelegationToolPreviewRow[\s\S]*?statusIcon\.icon\.shouldRender \? \([\s\S]*?name=\{statusIcon\.icon\.name\}[\s\S]*?color=\{statusIcon\.icon\.color\}[\s\S]*?export function ChatMessageDelegationMorePreviewAction/);
...
test('uses shared runtime presentation for mobile scroll-to-bottom affordance', () => {
  ...
  assert.match(
    chatMessageChromeSource,
    /export function ChatMessageScrollToBottomButtonTouchable\([\s\S]*?style=\{style\}[\s\S]*?onPress=\{onPress\}[\s\S]*?activeOpacity=\{activeOpacity\}[\s\S]*?accessibilityRole=\{accessibilityRole\}[\s\S]*?accessibilityLabel=\{accessibilityLabel\}[\s\S]*?accessibilityHint=\{accessibilityHint\}[\s\S]*?export function ChatMessageScrollToBottomButtonIcon/
  );
  assert.match(
    chatMessageChromeSource,
    /export function ChatMessageScrollToBottomButtonIcon\([\s\S]*?name=\{name\}[\s\S]*?size=\{size\}[\s\S]*?color=\{color\}[\s\S]*?export function ChatMessageLoadingState/
  );
