...
// @ts-ignore - uuid has no declaration file in this configuration
...
import type { AgentProgressUpdate, SessionProfileSnapshot, ConversationMessage, AgentProfile } from '../types';
import type { ACPDelegationProgress, ACPSubAgentMessage } from '@dotagents/shared';
...
/**
 * Minimal interface for the agent session tracker methods used by internal-agent.
 */
export interface InternalAgentSessionTracker {
...
let _agentSessionTracker: InternalAgentSessionTracker | null = null

/**
 * Set the ProgressEmitter for internal agent sub-sessions.
 */
export function setInternalAgentProgressEmitter(emitter: ProgressEmitter): void {
  _progressEmitter = emitter
}

/**
 * Set the agent session tracker for internal agent sub-sessions.
 */
export function setInternalAgentSessionTracker(tracker: InternalAgentSessionTracker): void {
...
/** Helper to get session profile snapshot from injected tracker */
const agentSessionTracker = {
  getSessionProfileSnapshot(sessionId: string): SessionProfileSnapshot | undefined {
    return _agentSessionTracker?.getSessionProfileSnapshot(sessionId) as SessionProfileSnapshot | undefined
  },
};
