/**
 * MCP Sampling — platform-agnostic sampling request management.
 *
 * Uses UserInteraction + ProgressEmitter instead of BrowserWindow.
 */

import { diagnosticsService } from "./diagnostics"
import { configStore } from "./config"
import type { SamplingRequest, SamplingResult, SamplingMessageContent } from "./types"
...
// Injected progress emitter for UI communication
let _progressEmitter: ProgressEmitter | null = null

/**
 * Set the ProgressEmitter adapter for sampling UI notifications.
 */
export function setSamplingProgressEmitter(emitter: ProgressEmitter): void {
...
export async function requestSampling(
  ...
  if (config.mcpRequireApprovalBeforeToolCall) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (_progressEmitter) {
        _progressEmitter.emitEvent("mcp:sampling-request", request)
      } else {
