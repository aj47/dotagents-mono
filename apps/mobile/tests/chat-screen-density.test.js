...
test('derives tool execution card status from displayed non-meta tool entries', () => {
  ...
  assert.doesNotMatch(chatMessageChromeSource, /getToolExecutionCompactMobileRenderState,/);
  assert.match(chatMessageChromeSource, /getChatRuntimeToolExecutionCompactPreviewMobileRowState,/);
  ...
  assert.match(sessionPresentationSource, /rows: \{\s+entries: renderedToolEntries,\s+stableMessageKey,\s+expandedToolCalls,\s+previewNumberOfLines,\s+pendingResultRenderState,\s+onToggleToolCall,/);
  assert.match(sessionPresentationSource, /compact: \{\s+onToggle: \(\) => onToggleMessageExpansion\(messageIndex\),\s+\},\s+expanded: \{\s+onToggle: \(\) => onToggleMessageExpansion\(messageIndex\),\s+emptyStateRenderState,\s+\},/);
  ...
  assert.match(chatMessageChromeSource, /const stackRenderState = getChatRuntimeToolExecutionStackMobileRenderState\(\{[\s\S]*?displayToolCallCount,[\s\S]*?results: rowInput\.entries\.map\(entry => entry\.result\),[\s\S]*?colors,[\s\S]*?emptyStateRenderState,/);
  ...
  assert.match(chatMessageChromeSource, /shouldRender: stackRenderState\.shouldRender,[\s\S]*?\.\.\.stackRenderState\.compact,[\s\S]*?\.\.\.stackRenderState\.expanded,/);
  ...
  assert.match(sessionPresentationSource, /expanded: \{\s+onToggle: \(\) => onToggleMessageExpansion\(messageIndex\),\s+emptyStateRenderState,/);
  ...
  assert.match(chatMessageChromeSource, /const rowState = getChatRuntimeToolExecutionDetailMobileRowState\(\{[\s\S]*?key,[\s\S]*?toolCall,[\s\S]*?label,[\s\S]*?result,[\s\S]*?isExpanded,[\s\S]*?colors,[\s\S]*?previewNumberOfLines,[\s\S]*?pendingResultRenderState,/);
  assert.match(chatMessageChromeSource, /return \{[\s\S]*?\.\.\.rowState,[\s\S]*?onHeaderPress: onToggle,/);
  ...
  assert.match(chatMessageChromeSource, /result: resultSection \? \{\s+\.\.\.resultSection,\s+onCopyPress: \(\) => onCopyPayload\(resultSection\.resultContent\),/);
  ...
  assert.match(chatMessageChromeSource, /input: input \? \{\s+\.\.\.input,\s+onCopyPress: \(\) => onCopyPayload\(input\.content\),/);
  ...
  assert.match(sessionPresentationSource, /onCopyPayload: \(content\) => \{ void onCopyPayload\(content\) \},/);
  assert.match(chatMessageChromeSource, /onCopyPress: \(\) => onCopyPayload\(input\.content\),/);
  assert.match(chatMessageChromeSource, /onCopyPress: \(\) => onCopyPayload\(resultSection\.resultContent\),/);
  assert.match(chatMessageChromeSource, /onErrorCopyPress: \(\) => onCopyPayload\(resultSection\.error \?\? ''\),/);
  assert.match(chatMessageChromeSource, /accessibilityRole=\{renderState\.accessibilityRole\}/);
  assert.match(chatMessageChromeSource, /accessibilityLabel=\{renderState\.accessibilityLabel\}/);
...
test('uses desktop-style streaming response chrome while mobile assistant content is live', () => {
  ...
  assert.match(sessionPresentationSource, /\.\.\.getChatRuntimeConversationContentMobileState\(\{\s+messageIndex,\s+visibleMessageContent,\s+isStreaming: isLiveStreamingAssistantMessage,\s+colors,\s+assetBaseUrl,\s+assetAuthToken,\s+spinnerSource,\s+onToggleMessageExpansion,/);
