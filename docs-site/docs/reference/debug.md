...
- `/mcp-tools` to list MCP and runtime tools grouped by source
- `/mcp-tool-show <tool-name-prefix>` to inspect one MCP or runtime tool in detail
- `/mcp-tool-enable <tool-name-prefix>` to enable one MCP or runtime tool
- `/mcp-tool-disable <tool-name-prefix>` to disable one MCP or runtime tool
- `/mcp-source-enable <source-name-prefix>` to enable all tools for one MCP source
- `/mcp-source-disable <source-name-prefix>` to disable all tools for one MCP source
- `/mcp-oauth` to list streamable HTTP MCP servers with OAuth status
- `/mcp-oauth-show <server-name-prefix>` to inspect OAuth status and token expiry for one MCP server
- `/mcp-oauth-start <server-name-prefix>` to start an OAuth flow and print a headless-safe authorization URL/state
- `/mcp-oauth-complete <server-name-prefix> <code> <state>` to finish an OAuth flow from a callback code/state
- `/mcp-oauth-revoke <server-name-prefix>` to clear stored OAuth tokens for one MCP server
...
The MCP OAuth commands above now also share the same MCP OAuth-management path that the desktop capabilities UI uses through `tipc.ts`, so streamable HTTP server filtering, auth-status inspection, headless-safe flow initiation, manual code/state completion, and token revocation stay aligned before terminal or GUI presentation diverges.
