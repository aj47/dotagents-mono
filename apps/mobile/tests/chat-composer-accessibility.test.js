...
test('uses shared session presentation for mobile composer copy and disabled state', () => {
  ...
  assert.match(chatMessageChromeSource, /<ChatComposerTextEntryInput\s+\{\.\.\.textEntryInput\.props\}/);
  assert.match(chatMessageChromeSource, /<ChatComposerTextEntryInputDescription\s+\{\.\.\.textEntryInputDescription\.props\}/);
  assert.match(chatMessageChromeSource, /<ChatComposerTextEntryVoiceStatusLiveRegion\s+\{\.\.\.textEntryVoiceStatusLiveRegion\.props\}/);
});
...
test('uses shared mobile composer control accessibility state', () => {
  ...
  assert.match(iconButtonSource, /<ChatComposerIconButtonIcon\s+\{\.\.\.icon\.props\}/);
  assert.match(sessionPresentationSource, /accessibilityRole: renderState\.accessibilityRole/);
  assert.match(sessionPresentationSource, /accessibilityLabel: renderState\.accessibilityLabel/);
...
test('keeps the chat composer send and queue controls icon-first with mobile touch targets', () => {
  ...
  assert.match(actionButtonSource, /<ChatComposerLabeledActionButtonTouchableContent\s+\{\.\.\.actionButtonTouchable\.content\}/);
  assert.match(actionButtonSource, /label\.shouldRender \? \(/);
  assert.match(actionButtonSource, /<ChatComposerLabeledActionButtonLabel\s+\{\.\.\.label\.props\}/);
  assert.doesNotMatch(actionButtonSource, /const touchableContent = actionButtonParts\.touchable\.content;/);
