 ...
 *
 * Legacy profile management — superseded by agent-profile-service.ts.
 * Extracted to @dotagents/core for completeness.
 */

import path from "path"
import fs from "fs"
import type { ProfileMcpServerConfig, ProfileModelConfig, ProfileSkillsConfig } from "./types"
...
/**
 * Set the builtin tool names provider for profile service.
 */
export function setProfileBuiltinToolNamesProvider(fn: () => string[]): void {
  _getBuiltinToolNamesForProfile = fn
}

// Legacy types - using permissive records for compatibility
...
class ProfileService {
  ...
  private loadProfiles(): ProfilesData {
    try {
      if (fs.existsSync(getProfilesPath())) {
        const data = JSON.parse(fs.readFileSync(getProfilesPath(), "utf8")) as ProfilesData
    ...
    const builtinToolNames = _getBuiltinToolNamesForProfile()
  ...
  private saveProfiles(): void {
    ...
    try {
      const dataFolder = path.dirname(getProfilesPath())
      fs.mkdirSync(dataFolder, { recursive: true })
      fs.writeFileSync(getProfilesPath(), JSON.stringify(this.profilesData, null, 2))
    } catch (error) {
  ...
  getProfile(id: string): Profile | undefined {
    return this.getProfiles().find((p: any) => p.id === id)
  }
  ...
  deleteProfile(id: string): boolean {
    ...
    this.profilesData!.profiles = this.profilesData!.profiles.filter((p: any) => p.id !== id)
    this.saveProfiles()
    return true
  }
  ...
  toggleProfileSkill(profileId: string, skillId: string): Profile {
    ...
    const newEnabledSkillIds = isCurrentlyEnabled
      ? currentEnabledSkills.filter((id: any) => id !== skillId)
