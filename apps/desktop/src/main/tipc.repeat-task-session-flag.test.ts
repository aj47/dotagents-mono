...
describe("getAgentSessions repeat-task session flag", () => {
  it("marks repeat-task sessions even if the visible title was retitled", () => {
    ...
    expect(tipcSource).toContain("isRepeatTaskSessionForTasks(session, loops)")
    expect(tipcSource).toContain("getFirstNonEmptyStoredServerConversationUserMessageContent(conversation)")
    expect(tipcSource).toContain("isRepeatTaskSessionForTasks(session, loops, { firstUserMessage })")
