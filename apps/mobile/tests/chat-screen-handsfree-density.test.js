...
test('renders the extracted handsfree status chip in the mobile chat composer', () => {
  ...
  assert.match(chatMessageChromeSource, /const handsFreeControlsRenderState = getChatComposerRuntimeHandsFreeControlsMobileRenderState\(\{[\s\S]*?phase: handsFreeStatusPhase,[\s\S]*?label: handsFreeStatusLabel,[\s\S]*?isEnabled: handsFreeStatusEnabled,[\s\S]*?wakePhrase: handsFreeStatusWakePhrase,[\s\S]*?sleepPhrase: handsFreeStatusSleepPhrase,[\s\S]*?lastError: handsFreeStatusLastError,[\s\S]*?foregroundOnly: handsFreeStatusForegroundOnly,[\s\S]*?\}\);/);
  assert.match(chatMessageChromeSource, /status: handsFreeControlsRenderState\.status,/);
...
test('keeps wake/sleep controls inline and wires a dedicated pause/resume control button', () => {
  ...
  assert.match(chatMessageChromeSource, /const handsFreeControlsRenderState = getChatComposerRuntimeHandsFreeControlsMobileRenderState\(\{/);
  ...
  assert.match(chatMessageChromeSource, /handsFreeControls: \{[\s\S]*?isVisible: controlRenderState\.visibility\.handsFreeControls\.isVisible,[\s\S]*?status: handsFreeControlsRenderState\.status,[\s\S]*?controlState: handsFreeControlsRenderState\.controlState,[\s\S]*?onWake: onWakeHandsFree,[\s\S]*?onSleep: onSleepHandsFree,[\s\S]*?onResume: onResumeHandsFree,[\s\S]*?onPause: onPauseHandsFree,[\s\S]*?\.\.\.chrome\.handsFreeControls,/);
  ...
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeControlMobileRenderState\(\{[\s\S]*?micPhase: composerControlMicPhase,[\s\S]*?listening: composerControlListening,[\s\S]*?colors: composerControlColors,[\s\S]*?\}\)/);
