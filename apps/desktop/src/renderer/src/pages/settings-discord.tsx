...
export function Component() {
  ...
  const saveConfig = useCallback((partial: Partial<Config>) => {
    ...
    saveConfigMutation.mutate({ config: { ...cfgRef.current, ...partial } })
  }, [saveConfigMutation])
          ...
          <div className="px-3 pb-3">
            ...
            {(statusError || status?.lastError) && (
              <div className="mb-3 flex items-start gap-2 rounded-md bg-amber-500/10 p-3 text-sm text-amber-600 dark:text-amber-400">
                ...
                <div>{statusError || status?.lastError}</div>
              </div>
            )}

            <div className="flex flex-wrap gap-2">
              <Button onClick={() => void handleConnect()} disabled={!enabled}>Connect</Button>
              <Button variant="outline" onClick={() => void handleDisconnect()} disabled={!enabled}>Disconnect</Button>
