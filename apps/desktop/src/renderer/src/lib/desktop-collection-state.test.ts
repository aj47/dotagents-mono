...
const agentStoreSource = readFileSync(new URL("../stores/agent-store.ts", import.meta.url), "utf8")
...
describe("desktop collection state helpers", () => {
  it("uses shared helpers for desktop renderer selection and expansion state", () => {
    ...
    expect(agentStoreSource).toContain("pinnedSessionIds: toggleSetValue(state.pinnedSessionIds, sessionId)")
    expect(agentStoreSource).toContain("archivedSessionIds: toggleSetValue(state.archivedSessionIds, sessionId)")
    ...
    expect(agentStoreSource).not.toContain("const newPinned = new Set(state.pinnedSessionIds)")
    expect(agentStoreSource).not.toContain("const newArchived = new Set(state.archivedSessionIds)")
  })
})
