...
describe("getConfiguredSttModel", () => {
  ...
  it("falls back to openai default when openaiSttModel is empty", () => {
    const config = { sttProviderId: "openai", openaiSttModel: "" } as const
  ...
  it("falls back to groq default when groqSttModel is empty", () => {
    const config = { sttProviderId: "groq", groqSttModel: "  " } as const
    expect(getConfiguredSttModel(config)).toBe("whisper-large-v3-turbo")
  })

  it("returns undefined for unknown provider", () => {
    const config = { sttProviderId: "parakeet" } as const
...
describe("resolveSttProviderId", () => {
  it("defaults to openai", () => {
    expect(resolveSttProviderId({})).toBe("openai")
  })

  it("preserves configured local providers", () => {
    expect(resolveSttProviderId({ sttProviderId: "parakeet" })).toBe("parakeet")
  })
})

describe("resolveSttModelSelection", () => {
  it("returns the default openai selection when unset", () => {
    expect(resolveSttModelSelection({})).toEqual({
      providerId: "openai",
      model: "whisper-1",
    })
  })

  it("returns the configured groq selection", () => {
    expect(
      resolveSttModelSelection({
        sttProviderId: "groq",
        groqSttModel: "whisper-large-v3",
  ...
  it("keeps parakeet selected without inventing a cloud model", () => {
    expect(
      resolveSttModelSelection({
  ...
  it("supports provider overrides for callers that need a specific cloud fallback", () => {
    expect(resolveSttModelSelection({}, "groq")).toEqual({
      providerId: "groq",
