...
describe("active agents sidebar always-on strip", () => {
  it("surfaces latest activity details and queue counters", () => {
    ...
    expect(sidebarSource).toContain("function getAlwaysOnQuestionContext")
    expect(sidebarSource).toContain("question.recommendation")
    expect(sidebarSource).toContain("getAlwaysOnQuestionCustomPlaceholder")
