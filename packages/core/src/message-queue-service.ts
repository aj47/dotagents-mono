...
import type { QueuedMessage, MessageQueue } from "@dotagents/shared"
import { logApp } from "./debug"
import type { ProgressEmitter } from "./interfaces/progress-emitter"

// Module-level ProgressEmitter for emitting queue updates to UI
let _progressEmitter: ProgressEmitter | null = null

/**
 * Set the ProgressEmitter implementation. Must be called at startup.
 */
export function setMessageQueueServiceProgressEmitter(emitter: ProgressEmitter): void {
...
export class MessageQueueService {
  ...
  /**
   * Emit queue update to UI via ProgressEmitter
   */
  private emitQueueUpdate(conversationId: string): void {
    ...
    try {
      _progressEmitter.emitQueueUpdate({
        conversationId,
        queue,
        isPaused,
      })
