...
describe('continuation guard helpers', () => {
  ...
  it('rejects tool placeholders with trailing content', () => {
    expect(isDeliverableResponseContent('[Calling tools: mark_work_complete] some trailing text')).toBe(false);
  });

  it('rejects garbled tool-call-as-text output', () => {
    expect(isDeliverableResponseContent(
      '[Calling tools: multi_tool_use.parallel] to=multi_tool_use.parallel json\n{"tool_uses":[{"recipient_name":"functions.save_note","parameters":{"body":"test"}}]}',
    )).toBe(false);
    expect(isDeliverableResponseContent(
      '[Calling tools: respond_to_user] to=functions.respond_to_user json\n{"text":"some response"}',
    )).toBe(false);
    expect(isDeliverableResponseContent(
