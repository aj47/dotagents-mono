...
import { configStore, getDataFolder, trySaveConfig } from "./config"
import type {
  MCPConfig,
  MCPServerConfig,
  MCPTransportType,
...
// ============================================================================
// Dependency injection for platform-specific services
// ============================================================================

/** OAuthClient-like interface for core */
...
/** OAuthStorage-like interface for core */
...
let _oauthClientFactory: ((serverName: string, config: any) => MCPOAuthClient) | null = null
let _oauthStorage: MCPOAuthStorage | null = null

export function setMCPServicePathResolver(resolver: PathResolver): void {
  _pathResolver = resolver
}

export function setMCPServiceUserInteraction(ui: UserInteraction): void {
  _userInteraction = ui
}

export function setMCPServiceOAuthFactory(factory: (serverName: string, config: any) => MCPOAuthClient): void {
  _oauthClientFactory = factory
}

export function setMCPServiceOAuthStorage(storage: MCPOAuthStorage): void {
  _oauthStorage = storage
}
...
export function getInternalWhatsAppServerPaths(): { scriptPath: string; nodeModulesPath: string } {
  const appPath = _pathResolver?.getAppPath?.() || process.cwd()
  if (process.env.NODE_ENV === "development" || process.env.ELECTRON_RENDERER_URL) {
    ...
    const resourcesDir = (process as any).resourcesPath || appPath
    return {
...
export class MCPService {
  ...
  > = new Map()
  private oauthClients: Map<string, MCPOAuthClient> = new Map()
  ...
  constructor() {
    ...
    try {
      ...
      const profilesPath = path.join(
        _pathResolver?.getAppDataPath?.() || getDataFolder(),
        process.env.APP_ID || "",
        "profiles.json"
      )
  ...
  private async createStreamableHttpTransport(serverName: string, serverConfig: MCPServerConfig): Promise<StreamableHTTPClientTransport> {
    ...
    const hasValidTokens = _oauthStorage ? await _oauthStorage.hasValidTokens(serverConfig.url) : false
  ...
  private async handle401AndRetryWithOAuth(serverName: string, serverConfig: MCPServerConfig): Promise<StreamableHTTPClientTransport> {
    ...
    try {
      ...
      // Store the tokens
      await _oauthStorage!.storeTokens(serverConfig.url, tokens)
   ...
   */
  private async getOrCreateOAuthClient(serverName: string, serverConfig: MCPServerConfig): Promise<MCPOAuthClient> {
    ...
    if (!oauthClient) {
      if (!_oauthClientFactory) {
        throw new Error("OAuth client factory not configured. Call setMCPServiceOAuthFactory() at app startup.")
      }
      // Load stored OAuth config
      const storedConfig = _oauthStorage ? await _oauthStorage.load(serverConfig.url) : {}
      const mergedConfig = { ...serverConfig.oauth, ...storedConfig }

      // Create new OAuth client
      oauthClient = _oauthClientFactory(serverConfig.url, mergedConfig)
  ...
  async initiateOAuthFlow(serverName: string): Promise<{ authorizationUrl: string; state: string }> {
    ...
    try {
      ...
      // Save updated config
      await _oauthStorage!.save(serverConfig.url, currentConfig)
  ...
  async completeOAuthFlow(serverName: string, code: string, state: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (clientConfig.clientId) {
        await _oauthStorage!.save(serverConfig.url, clientConfig)
      }
  ...
  async getOAuthStatus(serverName: string): Promise<{
    ...
    try {
      ...
      const hasValidTokens = _oauthStorage ? await _oauthStorage.hasValidTokens(serverConfig.url) : false
      const tokens = _oauthStorage ? await _oauthStorage.getTokens(serverConfig.url) : null
  ...
  async revokeOAuthTokens(serverName: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      // Clear stored tokens
      await _oauthStorage!.clearTokens(serverConfig.url)
  ...
  async executeToolCall(
    ...
    try {
      ...
      if (cfg.mcpRequireApprovalBeforeToolCall && !skipApprovalCheck) {
        ...
        const approved = _userInteraction
          ? await _userInteraction.requestApproval({
  ...
  async prepareEnvironment(
    ...
    if (serverName === "whatsapp") {
      ...
      environment.WHATSAPP_AUTH_DIR = path.join(_pathResolver?.getAppDataPath?.() || getDataFolder(), process.env.APP_ID || "dotagents", "whatsapp-auth")
    }
