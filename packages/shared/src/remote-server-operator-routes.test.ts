...
describe('remote server operator routes', () => {
  ...
  it('forwards status, query, params, audit contexts, and restart hooks to operator actions', async () => {
    ...
    await routes.get(`POST ${REMOTE_SERVER_API_ROUTE_PATHS.operatorStopTtsPlayback}`)!(
      createRequest(),
      createReply(),
    );
    ...
    expect(actions.snoozeOperatorAgentSession).toHaveBeenCalledWith('session-1');

    await routes.get(`POST ${REMOTE_SERVER_API_ROUTE_PATHS.operatorAgentSessionUnsnooze}`)!(
      createRequest({ params: { sessionId: 'session-1' } }),
      createReply(),
    );
    expect(actions.unsnoozeOperatorAgentSession).toHaveBeenCalledWith('session-1');

    await routes.get(`POST ${REMOTE_SERVER_API_ROUTE_PATHS.operatorAgentSessionClear}`)!(
      createRequest({ params: { sessionId: 'session-1' } }),
      createReply(),
    );
    expect(actions.clearOperatorAgentSession).toHaveBeenCalledWith('session-1');

    await routes.get(`POST ${REMOTE_SERVER_API_ROUTE_PATHS.operatorAgentSessionsClearInactive}`)!(
      createRequest(),
      createReply(),
    );
