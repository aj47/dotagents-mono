...
describe("desktop session copy failure feedback", () => {
  it("shows visible clipboard-copy failure feedback in AgentProgress", () => {
    ...
    expect(agentProgressSource).toContain('console.error(`Failed to copy ${label}:`, error)')
    expect(agentProgressSource).toContain('toast.error(getCopyErrorMessage(label, error))')
    expect(agentProgressSource).toContain('toast.error(getCopyErrorMessage("message", err))')
    expect(agentProgressSource).toContain('toast.error(getCopyErrorMessage("conversation", err))')
  })
