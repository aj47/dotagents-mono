...
- Mobile queued-message action-failure parity is now a strong follow-up because the mobile queue store also returns booleans for edit/remove/retry-style mutations, suggesting the same silent-failure trust gap may still exist there even though this iteration intentionally stayed on the desktop IPC-backed queue.
