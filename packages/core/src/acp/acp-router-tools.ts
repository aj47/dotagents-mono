...
/**
 * Set the ProgressEmitter for ACP router tools.
 */
export function setACPRouterToolsProgressEmitter(emitter: ProgressEmitter): void {
 ...
 */
export function getInternalAgentConfig(): import('../types').ACPAgentConfig {
  const internalInfo = getInternalAgentInfo();
  const config = configStore.get();

  // Check if user has explicitly disabled internal agent
  const userInternalConfig = config.acpAgents?.find((a: { name: string }) => a.name === 'internal');
...
export async function handleListAvailableAgents(args: {
  ...
  try {
    ...
    const legacyAcpAgents = (config.acpAgents || []).filter(
      (a: { name: string }) => a.name !== 'internal' && !agentTargetNames.has(a.name)
    );

    const formattedLegacyAcpAgents = legacyAcpAgents
      .filter((agent: { enabled?: boolean }) => agent.enabled !== false)
      .map((agent: { name: string; displayName?: string; description?: string; connection: { type: string }; enabled?: boolean }) => {
        const runtime = statusMap.get(agent.name);
        return {
          name: agent.name,
          displayName: agent.displayName,
...
function resolveAcpAgentConfig(agentName: string): ResolvedAcpAgentConfig | null {
  ...
  const config = configStore.get();
  const legacy = config.acpAgents?.find((a: { name: string }) => a.name === agentName);
  if (!legacy) {
    return null;
  }
