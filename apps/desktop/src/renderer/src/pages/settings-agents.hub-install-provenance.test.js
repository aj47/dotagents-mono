...
test('settings agents forwards Hub source provenance into the existing import dialog flow', () => {
  ...
  assert.match(settingsSource, /confirmLabel=\{prefilledImportSourceUrl \? "Install" : undefined\}/)
  assert.match(settingsSource, /successVerb=\{prefilledImportSourceUrl \? "installed" : undefined\}/)
...
test('bundle import dialog renders a visible external source link when provenance is available', () => {
  ...
  assert.match(dialogSource, /function buildSourceOutcomeMessage\(sourceLabel: string, sourceUrl\?: string\): string/)
  ...
  assert.match(dialogSource, /toast\.success\(`Successfully \$\{successVerb\} \$\{importSummary\.outcomeLabel\}\.\$\{detailMessage\}\$\{backupMessage\}\$\{sourceMessage\}`\)/)
  assert.match(dialogSource, /toast\.error\(`\$\{result\.errors\.join\(", "\) \|\| "Import failed"\}\.\$\{detailMessage\}\$\{backupMessage\}\$\{sourceMessage\}`\)/)
})