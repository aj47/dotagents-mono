...
test('renders delegated agent progress as compact desktop-style mobile chrome', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?lastConversationContentMessageIndex: lastAssistantContentMessageIndex,[\s\S]*?presentation: chatMessageConversationThreadPresentation,/);
...
test('uses shared runtime presentation for the mobile chat viewport and loading state', () => {
  ...
  assert.match(screenSource, /overlays=\{\{[\s\S]*?agentSelector: \{[\s\S]*?visible: agentSelectorVisible,[\s\S]*?onClose: \(\) => setAgentSelectorVisible\(false\),[\s\S]*?\}/);
...
test('uses shared desktop-style icons for mobile composer controls', () => {
  ...
  assert.match(screenSource, /imageAttachmentControl: \{\s+renderState: mobileComposerImageAttachmentRenderState,\s+onPress: handlePickImages,\s+\.\.\.chatComposerRuntimeDockChrome\.imageAttachmentControl,/);
  assert.match(screenSource, /textToSpeechControl: \{\s+renderState: mobileComposerTextToSpeechRenderState,\s+onPress: toggleTts,\s+\.\.\.chatComposerRuntimeDockChrome\.textToSpeechControl,/);
  assert.match(screenSource, /editBeforeSendControl: \{\s+shouldRender: mobileComposerVisibilityRenderState\.editBeforeSendControl\.shouldRender,\s+renderState: mobileComposerEditBeforeSendRenderState,\s+onPress: \(\) => setWillCancel\(\(current\) => !current\),\s+\.\.\.chatComposerRuntimeDockChrome\.editBeforeSendControl,/);
  ...
  assert.match(chatMessageChromeSource, /size=\{renderState\.icon\.size\}/);
  assert.match(chatMessageChromeSource, /color=\{renderState\.icon\.color\}/);
  assert.match(screenSource, /speechPreview: \{\s+label: mobileComposerControls\.sttPreview\.label,\s+text: sttPreview,\s+\}/);
  ...
  assert.match(screenSource, /queueAction: \{\s+shouldRender: mobileComposerVisibilityRenderState\.queueAction\.shouldRender,\s+renderState: mobileComposerQueueRenderState,\s+onPress: queueComposerInput,\s+\.\.\.chatComposerRuntimeDockChrome\.queueAction,/);
  ...
  assert.match(screenSource, /submitAction: \{\s+renderState: composerSubmitRenderState,\s+onPress: sendComposerInput,\s+\.\.\.chatComposerRuntimeDockChrome\.submitAction,/);
  ...
  assert.match(screenSource, /textEntry: \{\s+inputRef,\s+value: input,\s+onChangeText: handleInputChange,\s+onKeyPress: handleInputKeyPress,\s+accessibilityLabel: mobileComposerControls\.field\.accessibilityLabel,\s+accessibilityHint: composerAccessibilityHint,\s+placeholder: composerPlaceholder,\s+voiceStatusLiveRegionAnnouncement: voiceInputLiveRegionAnnouncement,\s+\.\.\.chatComposerRuntimeDockChrome\.textEntry,/);
  assert.match(screenSource, /voiceStatusLiveRegionAnnouncement: voiceInputLiveRegionAnnouncement/);
...
test('uses shared mobile icon chrome for pending image removal', () => {
  ...
  assert.match(screenSource, /pendingImagesRail: \{\s+images: pendingImages,\s+renderState: imageAttachmentRenderState,\s+onRemove: removePendingImage,\s+\}/);
...
test('derives visible assistant content from respond_to_user output and suppresses raw tool payloads', () => {
  ...
  assert.match(screenSource, /hasVisibleChatMessageContent,/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{\s+messages: visibleMessages,\s+firstMessageIndex: firstVisibleMessageIndex,\s+groupByIndex: toolActivityGroups\.groupByIndex,\s+groupState: expandedGroups,\s+inheritedState: expandedMessages,\s+onToggleGroup: toggleGroupExpansion,\s+lastConversationContentMessageIndex: lastAssistantContentMessageIndex,/);
  assert.match(chatMessageChromeSource, /return createChatMessageConversationItemThreadRenderState\(\{\s+\.\.\.threadInput,\s+group: groupByIndex\.get\(messageIndex\),\s+itemIndex: messageIndex,\s+itemKey: messageIndex,\s+message,\s+messageIndex,/);
  ...
  assert.match(screenSource, /<ChatMessageConversationRuntimeThreadList\s+threadStates=\{conversationThreadStates\}\s+styles=\{chatMessageConversationThreadStyles\.runtimeThread\}/);
...
test('uses shared media sanitization for collapsed mobile message previews', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?lastConversationContentMessageIndex: lastAssistantContentMessageIndex,/);
...
test('derives tool execution card status from displayed non-meta tool entries', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?messages: visibleMessages,[\s\S]*?lastConversationContentMessageIndex: lastAssistantContentMessageIndex,/);
...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationThreadListRenderState\(\{\s+messages: visibleMessages,\s+firstMessageIndex: firstVisibleMessageIndex,\s+groupByIndex: toolActivityGroups\.groupByIndex,\s+groupState: expandedGroups,\s+inheritedState: expandedMessages,\s+onToggleGroup: toggleGroupExpansion,\s+lastConversationContentMessageIndex: lastAssistantContentMessageIndex,/);
...
test('uses desktop-style streaming response chrome while mobile assistant content is live', () => {
  ...
  assert.match(screenSource, /const lastAssistantContentMessageIndex = findLastChatMessageConversationContentIndex\(\s+messages,\s+\(message\) => message,\s+\(message\) => hasVisibleChatMessageContent\(message\),\s+\);/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?lastConversationContentMessageIndex: lastAssistantContentMessageIndex,[\s\S]*?spinnerSource: isDark \? darkSpinner : lightSpinner,[\s\S]*?onToggleMessageExpansion: toggleMessageExpansion,/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /import \{[\s\S]*?ChatMessageConversationRuntimeThreadList,[\s\S]*?ChatMessageRuntimeSurface,[\s\S]*?createChatMessageConversationThreadListRenderState,[\s\S]*?createChatMessageConversationThreadPresentationState,[\s\S]*?createChatMessageConversationThreadStyleSlots,[\s\S]*?\} from '\.\.\/ui\/ChatMessageChrome';/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?lastConversationContentMessageIndex: lastAssistantContentMessageIndex,[\s\S]*?turnDurationsByUserTimestamp: turnDurations\.byUserTimestamp,/);
...
test('uses shared desktop chat message presentation tones for mobile message cards', () => {
  assert.match(screenSource, /getChatMessageMobileRenderState,/);
  assert.match(screenSource, /findLastChatMessageConversationContentIndex,/);
  assert.doesNotMatch(screenSource, /createChatMessageConversationRenderContext\(\{/);
  ...
  assert.match(screenSource, /<ChatMessageConversationRuntimeThreadList\s+threadStates=\{conversationThreadStates\}[\s\S]*?styles=\{chatMessageConversationThreadStyles\.runtimeThread\}/);
...
test('lets mobile edit and delete desktop saved prompts from quick-start cards', () => {
  ...
  assert.match(screenSource, /overlays=\{\{[\s\S]*?promptEditor: \{[\s\S]*?visible: addPromptModalVisible,/);
