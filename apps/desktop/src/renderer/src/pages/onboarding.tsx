...
import { resolveChatProviderId, resolveSttProviderId } from "@dotagents/shared"
...
export function Component() {
  ...
  const transcribeMutation = useMutation({
    mutationFn: async ({
      ...
      const isParakeet = resolveSttProviderId(config ?? {}) === "parakeet"
    ...
    onError: (error: any) => {
      ...
      if (
        ...
        setTranscriptionError(
          "API key is missing or invalid. Please go back and enter a valid Groq API key.",
        )
      } else if (errorMessage.includes("model")) {
        setTranscriptionError(
          "Model configuration error. Please check your settings.",
        )
      } else {
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        errorMessage.includes("no audio input")
      ) {
        setMicError(
...
function WelcomeStep({
  ...
  return (
    <div className="mx-auto max-w-xl text-center">
      <div className="mb-4">
        <span className="i-mingcute-mic-fill text-primary text-5xl sm:text-6xl"></span>
      </div>
      <h1 className="mb-3 text-2xl font-bold tracking-tight sm:text-3xl">
        Welcome to {process.env.PRODUCT_NAME}!
      </h1>
      <p className="text-muted-foreground mx-auto mb-6 max-w-xl text-base sm:text-lg">
        Let's get you set up with voice dictation and AI-powered tools in just a
        few steps.
      </p>
...
function DictationStep({
  ...
  const getShortcutDisplay = () => {
    if (shortcut === "hold-ctrl") {
      ...
      return mode === "hold"
        ? `Hold ${config.customShortcut}`
        : `Press ${config.customShortcut}`
    }
    return "Hold Ctrl"
  }
  ...
  const getButtonContent = () => {
    if (isTranscribing) {
      return {
        icon: "i-mingcute-loading-fill animate-spin",
        text: "Transcribing...",
      }
    }
    if (isRecording) {
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-center">
        Click the button or use your hotkey to record. Your speech will be
        transcribed below.
      </p>
      ...
      <div className="bg-muted/30 mb-6 rounded-lg border p-4">
        <div className="mb-3 flex items-center gap-2">
          <span className="i-mingcute-keyboard-fill text-primary text-lg"></span>
...
function AgentStep({
  ...
  const handleTestAgent = async () => {
    ...
    const providerId = resolveChatProviderId(config ?? {})
    const hasApiKey =
      providerId === "groq"
        ? !!config?.groqApiKey
        : providerId === "gemini"
    ...
    if (!hasApiKey) {
      setAgentError(
        `No API key configured. Please go back and enter your ${providerId === "groq" ? "Groq" : providerId === "gemini" ? "Gemini" : "OpenAI"} API key, or configure it in Settings.`,
      )
      return
    }
    ...
    try {
      ...
      setAgentPrompt("")
      setAgentResponse(
        "Agent started! Check the floating panel to see the progress.",
      )
    } catch (error: any) {
      ...
      if (errorMessage.includes("API key")) {
        ...
        setAgentError(
          "Model configuration error. Please check your model settings.",
        )
      } else {
  ...
  return (
    <div>
      ...
      <div className="bg-muted/30 mb-4 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="i-mingcute-search-fill text-primary text-2xl"></span>
            <div>
          ...
          >
            {isInstallingExa ? (
              <>
                <span className="i-mingcute-loading-fill mr-2 animate-spin"></span>
                Installing...
              </>
            ) : exaInstalled ? (
              <>
      ...
      <div className="bg-muted/30 mb-4 rounded-lg border p-4">
        <div className="mb-3 flex items-center gap-2">
          <span className="i-mingcute-robot-fill text-primary text-lg"></span>
          <label className="text-sm font-medium">Try Your Agent</label>
        </div>

        {/* Hotkey hint */}
        <div className="bg-primary/10 border-primary/20 mb-3 flex items-center gap-2 rounded border p-2">
          ...
          <span className="text-sm">
            ...
            to speak to your agent from anywhere
          </span>
        </div>
        ...
        {agentResponse && (
          <div className="mt-3 rounded border border-green-500/20 bg-green-500/10 p-2 text-sm text-green-600 dark:text-green-400">
        ...
        {agentError && (
          <div className="mt-3 rounded border border-red-500/20 bg-red-500/10 p-2 text-sm text-red-600 dark:text-red-400">
            <span className="i-mingcute-warning-fill mr-2"></span>
            {agentError}
          </div>
        )}
        ...
        <p className="text-muted-foreground mt-2 text-xs">
          Or use your hotkey to speak to the agent. Results will appear in the
          floating panel.
        </p>
      </div>
...
function FeatureCard({
  ...
  return (
    <div className="bg-muted/30 flex gap-4 rounded-lg border p-4">
      <div className="flex-shrink-0">
        <span className={`${icon} text-primary text-2xl`}></span>
      </div>
      <div>
