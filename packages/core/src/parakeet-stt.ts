 ...
 * Must be called before any STT operations.
 */
export function setParakeetSTTPathResolver(resolver: PathResolver): void {
  pathResolver = resolver
}

function requirePathResolver(): PathResolver {
  if (!pathResolver) {
    throw new Error("[Parakeet] PathResolver not set. Call setParakeetSTTPathResolver() before using STT.")
  }
  return pathResolver
}
...
function getSherpaLibraryPath(): string | null {
  ...
  if (resolver?.isPackaged?.()) {
    const resourcesPath = resolver.getResourcesPath?.()
    if (resourcesPath) {
      possiblePaths.push(
        path.join(resourcesPath, platformPackage)
      )
      // Legacy: also check node_modules in case it was bundled there
      possiblePaths.push(
        path.join(resourcesPath, "app", "node_modules", platformPackage)
      )
    }
  }
...
function getModelsPath(): string {
  return path.join(requirePathResolver().getUserDataPath(), "models", "parakeet")
}
