...
export class ConversationService {
  ...
  private normalizeTitleSource(titleSource: unknown): TitleSource | undefined {
    return isTitleSource(titleSource) ? titleSource : undefined
  }

  private canAutoGeneratedTitleReplace(conversation: Conversation): boolean {
    const titleSource = this.normalizeTitleSource(conversation.titleSource)
    if (titleSource) {
      return isFallbackTitleSource(titleSource)
    }
  ...
  private getAutoTitleSeed(conversation: Conversation): {
    ...
    if (!this.canAutoGeneratedTitleReplace(conversation)) {
      return null
    }
  ...
  async renameConversationTitle(conversationId: string, title: string, titleSource: TitleSource = "manual"): Promise<Conversation | null> {
    const normalizedTitle = this.normalizeConversationTitle(title)
    if (!normalizedTitle) {
      return null
    }
    const normalizedTitleSource = this.normalizeTitleSource(titleSource) ?? "manual"
    ...
    return this.enqueueConversationMutation(conversationId, async () => {
      ...
      if (
        this.normalizeConversationTitle(conversation.title) === normalizedTitle &&
        this.normalizeTitleSource(conversation.titleSource) === normalizedTitleSource
      ) {
        return conversation
      }

      conversation.title = normalizedTitle
      conversation.titleSource = normalizedTitleSource
      await this.saveConversationUnlocked(conversation)
      return conversation
    })
  }
  ...
  async maybeAutoGenerateConversationTitle(conversationId: string, sessionId?: string): Promise<Conversation | null> {
    ...
    return this.enqueueConversationMutation(conversationId, async () => {
      ...
      latestConversation.titleSource = "server_generated"
