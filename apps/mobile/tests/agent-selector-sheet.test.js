...
test('keeps current state visible when the active agent is missing from the loaded selector options', () => {
  ...
  assert.match(sheetSource, /currentSelectionNoticeButton:\s*\{[\s\S]*?\.\.\.actionButtonTouchTarget,[\s\S]*?minWidth:\s*180,[\s\S]*?backgroundColor:\s*theme\.colors\.primary \+ '12'/);
