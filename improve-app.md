...
### Checked Recently
- 2026-03-08: Mobile `ChatScreen` message-action touch-target/accessibility polish in `apps/mobile/src/screens/ChatScreen.tsx`, with the existing message-collapse, tool-summary, and per-message read-aloud affordances reviewed there, backlog context rechecked in `mobile-app-improvement.md`, adjacent mobile chat accessibility/source guardrails cross-checked in `apps/mobile/tests/chat-composer-accessibility.test.js` and `apps/mobile/tests/chat-message-tts-feedback.test.js`, focused source-level coverage added in new `apps/mobile/tests/chat-message-actions-accessibility.test.js`, targeted verification run locally via `node --test ...` plus `git diff --check`, live mobile inspection attempted via `pnpm --filter @dotagents/mobile exec expo --version` *(blocked because `expo` is unavailable in this dependency-light worktree)*, and broader mobile TypeScript verification attempted via `pnpm exec tsc -p apps/mobile/tsconfig.json --noEmit --pretty false` *(blocked because this worktree cannot resolve `expo/tsconfig.base` or the React Native/Expo dependency graph)*.
- 2026-03-08: Mobile Settings reconnect warning suppression in `apps/mobile/src/screens/SettingsScreen.tsx`, with the repeated `⚠️ Failed to load: settings` follow-up from `mobile-app-improvement.md` investigated, remote settings fetch/error semantics rechecked in `apps/mobile/src/lib/settingsApi.ts`, connection readiness reviewed in `apps/mobile/src/store/tunnelConnection.ts`, focused source-level coverage added in new `apps/mobile/tests/settings-remote-loading-mobile.test.js` and adjacent mobile Settings guardrails re-run in `apps/mobile/tests/settings-connection-card-mobile.test.js`, `apps/mobile/tests/settings-loop-feedback-mobile.test.js`, `apps/mobile/tests/settings-loop-actions-mobile.test.js`, `apps/mobile/tests/settings-loop-mutation-feedback-mobile.test.js`, and `apps/mobile/tests/settings-memory-actions-mobile.test.js`, targeted verification run locally via `node --test ...` plus `git diff --check`, and live mobile inspection attempted conceptually but remained blocked because this dependency-light worktree has no `node_modules` / Expo runtime available for reconnect reproduction.
- 2026-03-08: Desktop `Settings → Skills` save-feedback / required-field polish in `apps/desktop/src/renderer/src/pages/settings-skills.tsx`, with the create/edit mutation flow reviewed there, `createSkill(...)` / `updateSkill(...)` save semantics rechecked in `apps/desktop/src/main/tipc.ts` and `apps/desktop/src/main/skills-service.ts` to confirm failures throw instead of returning falsey success, adjacent inline retry/draft-preservation patterns cross-checked in `apps/desktop/src/renderer/src/pages/memories.tsx` and `tests/desktop-settings-agents-draft-guardrails.test.js`, mobile parity checked in `apps/mobile/src/screens/SettingsScreen.tsx` *(no matching mobile code change needed because mobile only toggles existing skills for the current profile rather than authoring/editing desktop skill files)*, focused source-level coverage added in new `tests/desktop-settings-skills-save-feedback.test.js` with adjacent skills draft/delete guardrails re-run, targeted verification run locally via `node --test tests/desktop-settings-skills-save-feedback.test.js tests/desktop-settings-skills-unsaved-changes.test.js tests/desktop-settings-skills-delete-guardrails.test.js`, `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false`, plus `git diff --check`, and live desktop inspection attempted via `electron_execute` *(blocked because the renderer target at `http://localhost:5173/settings/agents` only exposed shell text while `#root` had `0` children, so the actual mounted Skills dialogs could not be exercised end-to-end)*.
- 2026-03-08: Desktop past-sessions dialog focus handoff polish in `apps/desktop/src/renderer/src/components/past-sessions-dialog.tsx` and `apps/desktop/src/renderer/src/components/app-layout.tsx`, with dialog/search/delete behavior re-reviewed in the dialog component, launcher wiring and shared open state rechecked in `apps/desktop/src/renderer/src/components/app-layout.tsx`, `apps/desktop/src/renderer/src/pages/sessions.tsx`, and `apps/desktop/src/renderer/src/components/active-agents-sidebar.tsx`, Radix dialog/input ref support confirmed in `apps/desktop/src/renderer/src/components/ui/dialog.tsx` and `apps/desktop/src/renderer/src/components/ui/input.tsx`, mobile parity checked in `apps/mobile/src/screens/SessionListScreen.tsx` *(no matching mobile change needed because mobile uses a dedicated screen/list flow rather than a shared desktop modal dialog)*, focused source-level coverage extended in `tests/desktop-past-sessions-dialog-guardrails.test.js`, targeted verification run locally via `node --test tests/desktop-past-sessions-dialog-guardrails.test.js` plus `git diff --check`, desktop renderer typecheck attempted via `pnpm --filter @dotagents/desktop typecheck:web` *(blocked because this dependency-light worktree is missing `node_modules`, so `tsc` cannot resolve `@electron-toolkit/tsconfig/tsconfig.web.json`)*, and live desktop inspection attempted via `electron_execute` *(a renderer target existed at `http://localhost:5173/settings/agents`, but `#root` had `0` children so the actual mounted dialog UI could not be exercised)*.
- 2026-03-08: Desktop agent editor false-result save guardrails in `apps/desktop/src/renderer/src/pages/settings-agents.tsx`, with the save flow reviewed there, TIPC create/update return-paths rechecked in `apps/desktop/src/main/tipc.ts`, `agentProfileService.update(...)` falsey semantics confirmed in `apps/desktop/src/main/agent-profile-service.ts`, mobile parity checked in `apps/mobile/src/screens/AgentEditScreen.tsx` plus `apps/mobile/src/lib/settingsApi.ts` *(no matching mobile change needed because remote request failures already throw on non-OK HTTP responses instead of silently returning falsey save results)*, focused source-level coverage extended in `tests/desktop-settings-agents-draft-guardrails.test.js`, targeted verification run locally via `node --test tests/desktop-settings-agents-draft-guardrails.test.js tests/desktop-settings-agents-delete-guardrails.test.js`, `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false`, plus `git diff --check`, and live desktop inspection attempted via `electron_execute` *(blocked because the only available renderer target in this harness was the marketing site at `http://127.0.0.1:4312/`, not the desktop settings UI)*.
- 2026-03-08: Desktop MCP approval-dialog failure recovery in `apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx` and `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx`, with false-return semantics rechecked in `apps/desktop/src/main/mcp-elicitation.ts`, `apps/desktop/src/main/mcp-sampling.ts`, and `apps/desktop/src/main/tipc.ts`, adjacent destructive/save-recovery guardrail patterns cross-checked in `tests/desktop-mcp-server-save-guardrails.test.js`, focused source-level coverage added in new `tests/desktop-mcp-approval-dialog-feedback.test.js`, targeted verification run locally via `node --test tests/desktop-mcp-approval-dialog-feedback.test.js tests/desktop-mcp-server-save-guardrails.test.js`, `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false`, plus `git diff --check`, and live desktop inspection attempted via `electron_execute` *(renderer target was available at `/settings/agents`, but no active MCP elicitation/sampling request could be triggered from this dependency-light harness, so end-to-end dialog exercise stayed blocked)*.
...
### Not Yet Checked
- 2026-03-08: Mobile `ChatScreen` message-action touch-target polish in `apps/mobile/src/screens/ChatScreen.tsx` still needs live Expo/native or Expo-web validation once the mobile toolchain is available, especially to confirm the new collapsed-message disclosure row feels obviously tappable under large text, hiding the redundant collapsed chevron above tool summaries does not reduce discoverability, and the taller tool/read-aloud affordances do not make dense assistant turns feel overly vertical on smaller phones.
- 2026-03-08: Desktop `Settings → Skills` save-feedback / required-field polish in `apps/desktop/src/renderer/src/pages/settings-skills.tsx` still needs live Electron validation once a runnable settings renderer is available, especially to confirm the inline save-error banner feels calm above the long instructions editor, `Retry create` / `Retry save` labels do not crowd the footer, blank-name / blank-instructions edit attempts feel clearly recoverable, and trimming the saved skill name does not surprise users who intentionally paste leading/trailing whitespace.
- 2026-03-08: Desktop `Settings → Agents` save-failure recovery in `apps/desktop/src/renderer/src/pages/settings-agents.tsx` still needs live Electron validation once a runnable settings renderer is available, especially to confirm the new inline save-error banner feels calm in the dense editor footer, stale-save retries do not crowd the action row, and falsey/stale-id save failures now keep the draft visibly intact instead of feeling like a silent close.
- 2026-03-08: Desktop MCP approval dialogs in `apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx` and `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx` still need live Electron validation with a real pending elicitation/sampling request, especially to confirm the new inline expired-request copy, preserved form draft visibility, pending-button labels, and `Close` fallback feel calm when a request times out or disappears mid-response.
...
- 2026-03-08: Mobile queued-message action-failure parity in `apps/mobile/src/ui/MessageQueuePanel.tsx` / `apps/mobile/src/store/message-queue.ts` still needs live Expo/native validation once the mobile toolchain is available, especially to confirm the new inline retry banners, pending-state labels, and compact clear-state hierarchy feel trustworthy on small screens after a real failed queue mutation.
...
- 2026-03-08: Desktop past-sessions dialog loading/search/focus recovery still needs live Electron validation once a runnable target is available, especially to confirm the blocking-load card, cached-list refresh-warning banner, `Retry loading` affordance, `Clear search` empty state, search autofocus, and launcher-focus restoration all feel calm without crowding the compact dialog.
...
### Improved
- 2026-03-08: Mobile `ChatScreen` in `apps/mobile/src/screens/ChatScreen.tsx` now puts collapsed-message preview text inside the actual expand/collapse pressable instead of leaving only a tiny chevron tappable, suppresses the redundant collapsed header when a tool-execution summary row already serves as the expansion affordance, aligns the visible/a11y copy for expanded tool-details collapse, and reuses one `messageActionTouchTarget` helper for collapsed message rows, tool-summary rows, tool-detail headers, and the per-message `Read aloud` button so the most frequently tapped inline message actions move closer to a reliable 44×44 target on mobile. Why it was worthwhile: the prior UI concentrated important message actions into tiny pills/chevrons that were easy to miss or fat-finger, especially in long assistant turns with tool output. Tradeoff: this pass intentionally stays inside `ChatScreen` rather than broadening into new message actions like copy/share or redesigning the broader mobile message card layout in the same iteration.
- 2026-03-08: Desktop `Settings → Skills` in `apps/desktop/src/renderer/src/pages/settings-skills.tsx` now keeps create/edit save failures visible inline instead of relying on transient toasts alone, preserves the current draft while surfacing retry-oriented error copy, clears stale errors as soon as the draft changes or a fresh dialog opens, blocks no-op saves until a draft is actually dirty, and applies the same required-field validation to edit saves that create already expected, so long skill-instruction drafts no longer feel like they vanished or saved successfully when a write failed or when the user accidentally blanked out a required field. Tradeoff: this pass intentionally keeps the change local to the renderer save flow instead of refactoring skill-file validation into shared services, and it trims saved skill names for consistency while leaving multi-line instructions untouched so author formatting is preserved.
- 2026-03-08: Desktop past-sessions dialog in `apps/desktop/src/renderer/src/components/past-sessions-dialog.tsx` now autofocuses the search field on open, and `apps/desktop/src/renderer/src/components/app-layout.tsx` now captures whichever control launched the dialog so focus can return there on dismiss, which makes keyboard-driven history browsing feel more predictable from the sidebar and sessions empty state instead of dropping users into an ambiguous focus location after close. Tradeoff: this pass intentionally kept the change scoped to open/dismiss focus behavior and left row-to-session handoff targeting plus broader keyboard-navigation changes for a later live-validation pass.
- 2026-03-08: Desktop `Settings → Agents` in `apps/desktop/src/renderer/src/pages/settings-agents.tsx` now treats falsey create/update save results as real failures instead of silently closing the editor, keeps the current draft open, shows inline retryable error guidance plus a toast, and clears stale save errors when opening or replacing drafts, so agent edits no longer appear to save successfully when the underlying profile update never actually committed. Tradeoff: this pass intentionally keeps the fix local to the renderer save flow instead of changing TIPC/service return shapes or broadening into a larger agent-management refactor.
- 2026-03-08: Desktop MCP approval dialogs in `apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx` and `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx` now keep failed or expired response attempts visible instead of silently closing, preserve elicitation form drafts when `resolveElicitation(...)` cannot complete, show inline alert copy plus explicit `Close` fallback when the request is no longer pending, and disable duplicate submit buttons while a response is in flight, so MCP approval prompts no longer feel like dead clicks or disappearing forms when the backend times out or IPC resolution fails. Tradeoff: this pass intentionally keeps recovery local to the renderer dialogs instead of changing the main-process approval APIs or introducing a broader shared approval-state abstraction.
...
### Verified
- 2026-03-08: `node --test apps/mobile/tests/chat-message-actions-accessibility.test.js apps/mobile/tests/chat-composer-accessibility.test.js apps/mobile/tests/chat-message-tts-feedback.test.js` after the mobile `ChatScreen` message-action touch-target/accessibility polish pass
- 2026-03-08: `pnpm exec tsc -p apps/mobile/tsconfig.json --noEmit --pretty false` after the mobile `ChatScreen` message-action touch-target/accessibility polish pass *(blocked: this dependency-light worktree cannot resolve `expo/tsconfig.base` or the React Native/Expo dependency graph, so broader mobile typecheck remains unavailable here)*
- 2026-03-08: `git diff --check` after the mobile `ChatScreen` message-action touch-target/accessibility polish pass
- 2026-03-08: `node --test tests/desktop-settings-skills-save-feedback.test.js tests/desktop-settings-skills-unsaved-changes.test.js tests/desktop-settings-skills-delete-guardrails.test.js` after the desktop skills save-feedback / required-field polish pass
- 2026-03-08: `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false` after the desktop skills save-feedback / required-field polish pass
- 2026-03-08: `git diff --check` after the desktop skills save-feedback / required-field polish pass
- 2026-03-08: `node --test tests/desktop-past-sessions-dialog-guardrails.test.js` after the desktop past-sessions dialog focus handoff polish pass
- 2026-03-08: `pnpm --filter @dotagents/desktop typecheck:web` after the desktop past-sessions dialog focus handoff polish pass *(blocked: this dependency-light worktree is missing `node_modules`, so `tsc` cannot resolve `@electron-toolkit/tsconfig/tsconfig.web.json`)*
- 2026-03-08: `git diff --check` after the desktop past-sessions dialog focus handoff polish pass
- 2026-03-08: `node --test tests/desktop-settings-agents-draft-guardrails.test.js tests/desktop-settings-agents-delete-guardrails.test.js` after the desktop agent-editor false-result save-guardrail pass
- 2026-03-08: `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false` after the desktop agent-editor false-result save-guardrail pass
- 2026-03-08: `git diff --check` after the desktop agent-editor false-result save-guardrail pass
- 2026-03-08: `node --test tests/desktop-mcp-approval-dialog-feedback.test.js tests/desktop-mcp-server-save-guardrails.test.js` after the desktop MCP approval-dialog failure recovery pass
- 2026-03-08: `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false` after the desktop MCP approval-dialog failure recovery pass
- 2026-03-08: `git diff --check` after the desktop MCP approval-dialog failure recovery pass
...
### Blocked
- 2026-03-08: Live mobile validation for this `ChatScreen` message-action touch-target pass is blocked because `pnpm --filter @dotagents/mobile exec expo --version` still reports that `expo` is unavailable in this dependency-light worktree, so the new collapsed-message disclosure row and taller tool/read-aloud affordances could only be validated via source inspection plus targeted guardrail tests.
- 2026-03-08: Broader mobile TypeScript verification for this `ChatScreen` message-action touch-target pass is blocked in this worktree because `pnpm exec tsc -p apps/mobile/tsconfig.json --noEmit --pretty false` cannot resolve `expo/tsconfig.base` or the React Native/Expo dependency graph without installed mobile dependencies.
- 2026-03-08: Live desktop validation for this `Settings → Skills` save-feedback / required-field polish pass is blocked in the current harness because `electron_execute` only exposed `http://localhost:5173/settings/agents` with page-shell text while `#root` had `0` children, so there was no mounted settings editor UI to exercise even though a renderer target technically existed.
- 2026-03-08: Live desktop validation for this past-sessions dialog focus handoff pass is blocked in the current harness because `electron_execute` only exposed `http://localhost:5173/settings/agents` with page-shell text while `#root` had `0` children, so there was no mounted dialog UI to exercise even though a renderer target technically existed.
- 2026-03-08: Focused desktop web typecheck for this past-sessions dialog focus handoff pass is blocked in this worktree because `node_modules` is missing, so `pnpm --filter @dotagents/desktop typecheck:web` cannot resolve `@electron-toolkit/tsconfig/tsconfig.web.json`.
- 2026-03-08: Live desktop validation for the new `Settings → Agents` save-failure recovery pass is blocked in this dependency-light harness because `electron_execute` only exposed the marketing-site renderer at `http://127.0.0.1:4312/` rather than an inspectable desktop settings UI, so this iteration relied on source inspection plus targeted save-flow guardrails instead of an end-to-end editor exercise.
- 2026-03-08: Live end-to-end desktop MCP approval-dialog validation for this failure-recovery pass is still blocked even though an Electron renderer target is available, because the current dependency-light harness could only inspect `/settings/agents` and did not provide a straightforward way to trigger a real pending elicitation/sampling request for timeout/false-return exercise, so this iteration relied on source inspection plus targeted dialog guardrails instead of a real MCP prompt round-trip.
...
- Desktop `Settings → Agents` save-failure recovery live validation / inline-banner density (`apps/desktop/src/renderer/src/pages/settings-agents.tsx`)
- Desktop MCP approval dialogs live validation with a real pending elicitation / sampling request (`apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx`, `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx`)
...
- Mobile `ChatScreen` now has source-level message-action touch-target guardrails, so the freshest adjacent follow-up is live Expo/native or Expo-web validation of collapsed-message discoverability, large-text density, and whether the same 44×44 treatment should also be extended to `ResponseHistoryPanel` / any future per-message copy action once the mobile toolchain is available.
- Desktop `Settings → Skills` now has source-level save-failure and required-field guardrails, so the freshest adjacent follow-up is live Electron validation of the inline error banner, retry-label density, footer affordances, and trimmed-name behavior once a runnable settings renderer is available.
- Desktop `Settings → Agents` now has source-level false-result save guardrails, so the freshest adjacent follow-up is live Electron validation of the inline save-error banner, retry cadence, and stale-id save-race behavior once a runnable settings renderer is available.
- Desktop MCP approval dialogs now have source-level failure/expired-request guardrails, so the freshest adjacent follow-up is live Electron validation with a real MCP elicitation and sampling request to confirm the inline alert density, preserved form-draft visibility, pending-button labels, and `Close` fallback all feel trustworthy under actual timeout and already-resolved timing.
...
- Mobile `MemoryEditScreen` now has source-level dirty-draft guardrails, so the freshest adjacent follow-up is live Expo/native validation of header-back, gesture-back, and Android hardware-back discard flows plus small-screen layout density around the tags field and save CTA.
...
- Desktop past-sessions dialog now has source-level search autofocus plus launcher-focus restoration guardrails, so the freshest desktop UX follow-up is live Electron validation of row-to-session handoff, keyboard traversal, and close/reopen focus behavior once a mounted renderer target is available.
...
### 2026-03-08 — Desktop past-sessions dialog focus handoff polish
- Date:
  - 2026-03-08
- Area / screen / subsystem:
  - desktop past-sessions history dialog in `apps/desktop/src/renderer/src/components/past-sessions-dialog.tsx`
  - shared launcher wiring in `apps/desktop/src/renderer/src/components/app-layout.tsx`
  - session entry points reviewed in `apps/desktop/src/renderer/src/pages/sessions.tsx` and `apps/desktop/src/renderer/src/components/active-agents-sidebar.tsx`
  - focused source-level guardrails extended in `tests/desktop-past-sessions-dialog-guardrails.test.js`
- Why it was chosen:
  - the ledger still called out past-sessions focus restoration as one of the freshest desktop UX gaps after earlier loading/search/delete safety passes
  - investigation found the dialog is controlled outside a `DialogTrigger`, so there was no explicit source-level guarantee that keyboard focus would land in search on open or return to the launching control on dismiss
  - this was a small, local fix with clear accessibility value and a straightforward verification path in a dependency-light worktree
- What was inspected:
  - `apps/desktop/src/renderer/src/components/past-sessions-dialog.tsx` for dialog open/close behavior, search affordances, and destructive states
  - `apps/desktop/src/renderer/src/components/app-layout.tsx`, `apps/desktop/src/renderer/src/pages/sessions.tsx`, and `apps/desktop/src/renderer/src/components/active-agents-sidebar.tsx` to confirm all current desktop launch paths funnel through shared dialog state
  - `apps/desktop/src/renderer/src/components/ui/dialog.tsx` and `apps/desktop/src/renderer/src/components/ui/input.tsx` to verify `DialogContent` focus hooks and input refs were available before editing
  - live renderer state via `electron_execute`, which exposed `http://localhost:5173/settings/agents` but only an unhydrated shell (`#root` child count `0`), so meaningful dialog interaction stayed blocked
- Improvement made:
  - added an optional `restoreFocusRef` to `PastSessionsDialog`
  - autofocuses the search input on open via `onOpenAutoFocus`
  - restores focus to the captured launcher on dismiss via `onCloseAutoFocus`
  - captures the active launcher element in `app-layout` before opening the dialog and reuses that handler across the collapsed sidebar, expanded sidebar, and outlet-driven sessions entry points
  - extended source guardrails to assert both the dialog-side autofocus/restore hooks and the launcher-capture wiring
- Assumptions / tradeoffs / rationale:
  - assumed the highest-value polish was predictable open/dismiss focus behavior for keyboard users, not a broader rewrite of row-level keyboard traversal in the same iteration
  - kept the change in the shared launcher path instead of threading individual refs through each dialog entry point, which kept the fix local and easier to reason about
  - left row-to-session handoff focus targeting for a later live pass because the best destination element after opening a history item depends on the mounted session surface and is difficult to validate reliably from source alone
- Tests / verification:
  - `node --test tests/desktop-past-sessions-dialog-guardrails.test.js`
  - `pnpm --filter @dotagents/desktop typecheck:web` *(blocked: `node_modules` missing, so `tsc` could not resolve `@electron-toolkit/tsconfig/tsconfig.web.json`)*
  - `git diff --check`
- Follow-up checks:
  - once a mounted Electron renderer is available, live-check keyboard open/close cycles from the sidebar and sessions empty state to confirm search autofocus and launcher restoration feel natural
  - validate row-to-session handoff when opening an existing session from the dialog so focus lands somewhere intentional rather than merely closing the modal
...
### 2026-03-08 — Desktop MCP approval-dialog failure recovery
- Area / screen / subsystem:
  - `apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx`
  - `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx`
  - `tests/desktop-mcp-approval-dialog-feedback.test.js`
- Why it was chosen:
  - after reviewing `improve-app.md`, many recent settings and draft-loss seams were already covered, so a fresh high-leverage opportunity remained in a core desktop approval flow that had not yet been investigated directly
  - source review found a concrete user-trust bug: both MCP approval dialogs closed immediately after `resolveElicitation(...)` / `resolveSampling(...)` even though the main process can return `false` when the pending request has already timed out or been cleared, which could silently discard an elicitation form draft or make approval clicks look successful when nothing happened
- What was inspected:
  - ledger history in `improve-app.md` to avoid repeating a just-touched area
  - renderer approval-dialog flows in `apps/desktop/src/renderer/src/components/mcp-elicitation-dialog.tsx` and `apps/desktop/src/renderer/src/components/mcp-sampling-dialog.tsx`
  - main-process pending-request/timeout semantics in `apps/desktop/src/main/mcp-elicitation.ts`, `apps/desktop/src/main/mcp-sampling.ts`, and `apps/desktop/src/main/tipc.ts`
  - adjacent inline-recovery and duplicate-submit guardrail patterns in `tests/desktop-mcp-server-save-guardrails.test.js`
  - live desktop inspection via `electron_execute`, which confirmed a runnable renderer target but no active MCP request harness for end-to-end dialog exercise
- Improvement made:
  - taught both dialogs to treat false resolve results as real failures instead of success, leaving the dialog open and surfacing inline alert copy rather than silently dismissing
  - preserved elicitation form drafts when submission fails or when the request has already expired, and swapped to an explicit `Close` fallback once the request is no longer actionable
  - disabled duplicate approval/decline/cancel clicks while a response is in flight and added clearer in-progress labels to reduce accidental double submission
  - added focused dependency-light guardrails in new `tests/desktop-mcp-approval-dialog-feedback.test.js`
  - assumptions / tradeoffs: this pass intentionally keeps the fix in the renderer layer instead of changing main-process resolve APIs, and it treats `false` responses as an unretryable “request is no longer pending” case rather than inventing a deeper retry protocol without evidence that one is needed
- Tests / verification:
  - `node --test tests/desktop-mcp-approval-dialog-feedback.test.js tests/desktop-mcp-server-save-guardrails.test.js`
  - `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false`
  - `git diff --check`
  - attempted live desktop inspection via `electron_execute` *(partial success: renderer target available at `/settings/agents`, but no active MCP elicitation/sampling request could be triggered from this harness for full dialog exercise)*
- Follow-up checks:
  - live-validate timeout / already-resolved MCP prompts with a real pending elicitation and sampling request once a reproducible desktop harness is available
  - inspect whether any other desktop approval surfaces still close on `false` or thrown resolve results without preserving local context

### 2026-03-08 — Desktop agent editor false-result save guardrails
- Date:
  - 2026-03-08
- Area / screen / subsystem:
  - `apps/desktop/src/renderer/src/pages/settings-agents.tsx`
- Why it was chosen:
  - `Settings → Agents` is a high-trust editing surface, and investigation showed the editor closed after `updateAgentProfile(...)` even though the underlying TIPC/service path can return a falsey result when the profile no longer exists, creating a silent save-loss failure.
- What was inspected:
  - renderer save flow in `apps/desktop/src/renderer/src/pages/settings-agents.tsx`
  - TIPC create/update handlers in `apps/desktop/src/main/tipc.ts`
  - `agentProfileService.update(...)` return semantics in `apps/desktop/src/main/agent-profile-service.ts`
  - mobile parity in `apps/mobile/src/screens/AgentEditScreen.tsx` and `apps/mobile/src/lib/settingsApi.ts` *(no equivalent change needed because mobile request failures already throw on non-OK HTTP responses instead of returning falsey save results)*
- Improvement made:
  - agent create/edit save now treats falsey create/update results as failures, keeps the draft open, shows inline retryable error copy plus a toast, and clears stale save errors when the user opens, closes, or replaces a draft.
- Tests / verification:
  - `node --test tests/desktop-settings-agents-draft-guardrails.test.js tests/desktop-settings-agents-delete-guardrails.test.js`
  - `pnpm exec tsc -p apps/desktop/tsconfig.json --noEmit --pretty false`
  - `git diff --check`
  - attempted live desktop inspection via `electron_execute` *(blocked because the only available renderer target in this harness was the marketing site at `http://127.0.0.1:4312/`, not the desktop settings UI)*
- Follow-up checks:
  - live-validate the inline save-error banner and retry flow in a real desktop `Settings → Agents` renderer
  - inspect nearby agent/task-management save flows for the same falsey-success pattern, especially any surfaces that close dialogs or navigate away immediately after save

### 2026-03-08 — Mobile Settings reconnect warning suppression
- Date:
  - 2026-03-08
- Area / screen / subsystem:
  - mobile Settings desktop-configuration sync in `apps/mobile/src/screens/SettingsScreen.tsx`
  - remote desktop settings client semantics in `apps/mobile/src/lib/settingsApi.ts`
  - connection readiness state in `apps/mobile/src/store/tunnelConnection.ts`
  - repeated reconnect warning evidence noted in `mobile-app-improvement.md`
- Why it was chosen:
  - the mobile improvement ledger still had a repeated `⚠️ Failed to load: settings` warning after reconnect on its unresolved list, which pointed to a concrete user-facing trust issue in a high-traffic settings surface
  - investigation showed a small local fix path: the Settings screen was firing remote desktop-settings requests whenever saved credentials existed, even while the connection manager still reported connecting or reconnecting
- What was inspected:
  - remote settings fetch / focus refresh flow in `apps/mobile/src/screens/SettingsScreen.tsx`
  - remote request helpers in `apps/mobile/src/lib/settingsApi.ts`
  - connection-state contract in `apps/mobile/src/store/tunnelConnection.ts`
  - adjacent mobile Settings source-level guardrails in `apps/mobile/tests/settings-connection-card-mobile.test.js`, `apps/mobile/tests/settings-loop-feedback-mobile.test.js`, `apps/mobile/tests/settings-loop-actions-mobile.test.js`, `apps/mobile/tests/settings-loop-mutation-feedback-mobile.test.js`, and `apps/mobile/tests/settings-memory-actions-mobile.test.js`
  - attempted live product inspection remained blocked in this workspace because `node_modules` / Expo are unavailable, so this pass relied on source investigation plus focused regression tests
- Improvement made:
  - the mobile Settings screen now only fetches remote desktop settings once the connection manager says the saved endpoint is actually `connected`
  - reconnect/disconnect transitions now invalidate in-flight remote settings requests so stale partial failures cannot overwrite the current screen state after the connection state changes
  - pull-to-refresh and focus-driven DotAgents follow-up fetches now skip reconnect/disconnected windows instead of spamming extra remote requests while the connection is still recovering
  - switching saved connection credentials now clears old remote desktop-settings data before the next eligible fetch so a previous server’s state does not linger across endpoint changes
- Assumptions / tradeoffs / rationale:
  - kept the fix scoped to request timing/state invalidation rather than a larger Settings UI redesign because the clearest user pain was the noisy reconnect warning, not the overall layout
  - intentionally preserved the existing connected-state fetch behavior and non-DotAgents detection so generic OpenAI-compatible endpoints still stay hidden without inventing new server-detection rules
  - chose request invalidation plus gating over more retry loops to avoid adding extra reconnect churn or background traffic while the connection manager is already recovering
- Tests / verification:
  - `node --test apps/mobile/tests/settings-remote-loading-mobile.test.js apps/mobile/tests/settings-connection-card-mobile.test.js apps/mobile/tests/settings-loop-feedback-mobile.test.js apps/mobile/tests/settings-loop-actions-mobile.test.js apps/mobile/tests/settings-loop-mutation-feedback-mobile.test.js apps/mobile/tests/settings-memory-actions-mobile.test.js`
  - `git diff --check`
- Follow-up checks:
  - live-verify the mobile reconnect path on Expo/device once dependencies are available, specifically confirming the spurious `Failed to load: settings` banner no longer appears after recovery
  - inspect whether the desktop-settings section should also visibly disable or hide stale controls while the connection is reconnecting, now that the warning spam path is fixed
