...
test('uses tool activities wording consistently for grouped tool activity labels', () => {
  ...
  assert.match(screenSource, /<ChatMessageRuntimeThread\s+key=\{i\}\s+groupRenderState=\{groupRenderState\}\s+onToggleGroup=\{group \? \(\) => toggleGroupExpansion\(group\) : undefined\}\s+styles=\{chatMessageRuntimeThreadStyles\}\s+body=\{createChatMessageConversationThreadBodyInput\(\{/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /import \{[\s\S]*?ChatMessageRuntimeThread,[\s\S]*?createChatMessageRuntimeThreadStyleSlots,[\s\S]*?createChatMessageToolActivityGroupThreadSurfaceStyleSlots,[\s\S]*?\} from '\.\.\/ui\/ChatMessageChrome';/);
...
test('uses shared desktop chat message presentation tones for mobile message cards', () => {
  ...
  assert.match(screenSource, /<ChatMessageRuntimeThread\s+key=\{i\}\s+groupRenderState=\{groupRenderState\}[\s\S]*?styles=\{chatMessageRuntimeThreadStyles\}/);
...
test('lets mobile branch linked desktop conversations from individual messages', () => {
  ...
  assert.match(screenSource, /actionStyles: messageActionStyles,/);
