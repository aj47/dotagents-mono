...
- Memory is layered: knowledge notes are canonical for durable facts (preferences, decisions, workflows, config), and recent conversation history is fresher working memory for current project state, blockers, and continuation/status tasks; for "continue"/"what happened"/"where are we"/"latest"/"status" style requests, prefer recent conversations (default last 7-14 days) and resolve conflicts by freshness, domain, and confidence
