...
test('lets mobile respond to desktop tool approval requests from progress updates', () => {
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationThreadBodyMobileState\(\{[\s\S]*?inlineActivity: createChatMessageInlineActivityProps\(\{/);
...
test('derives visible assistant content from respond_to_user output and suppresses raw tool payloads', () => {
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationMessageRuntimeThreadState\(\{\s+renderContext,\s+\.\.\.runtimeThreadInput,\s+\}\);/);
...
test('bases assistant collapse decisions on visible content instead of raw tool payload metadata', () => {
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationMessageRenderContextMobileState\(input\);/);
...
test('uses tool activities wording consistently for grouped tool activity labels', () => {
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationRuntimeThreadState\(\{\s+itemKey,\s+groupRenderState,\s+groupThreadState,\s+body,\s+\}\);/);
  ...
  assert.match(chatMessageChromeSource, /return getChatRuntimeConversationToolActivityGroupRuntimeThreadState\(runtimeThreadInput\);/);
