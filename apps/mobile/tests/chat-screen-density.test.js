...
test('derives tool execution card status from displayed non-meta tool entries', () => {
  ...
  assert.match(screenSource, /const toolExecutionRows = createChatMessageToolExecutionRows\(\{\s+entries: renderedToolEntries,\s+stableMessageKey,\s+expandedToolCalls,\s+colors: theme\.colors,\s+previewNumberOfLines: toolExecutionDetailStyleState\.payloadPreview\.numberOfLines,\s+pendingResultRenderState: toolExecutionDetailPendingResultState,\s+onToggleToolCall: toggleToolCallExpansion,\s+onCopyPayload: \(content\) => \{ void handleCopyToolPayload\(content\); \},\s+\}\);/);
  ...
  assert.match(chatMessageChromeSource, /const toolExecutionVisibilityRenderState = getToolExecutionMobileVisibilityRenderState\(\{\s+toolCallCount: displayToolCallCount,\s+\}\);/);
  assert.match(chatMessageChromeSource, /shouldRender: toolExecutionVisibilityRenderState\.toolPreview\.shouldRender,/);
  assert.match(screenSource, /toolExecutionStack: createChatMessageToolExecutionStackProps\(\{\s+displayToolCallCount,\s+colors: theme\.colors,\s+isExpanded,\s+compact: \{\s+rows: toolExecutionRows\.compactRows,\s+onToggle: \(\) => toggleMessageExpansion\(i\),/);
  ...
  assert.match(screenSource, /const toolExecutionRows = createChatMessageToolExecutionRows\(\{\s+entries: renderedToolEntries,\s+stableMessageKey,\s+expandedToolCalls,/);
  ...
  assert.match(screenSource, /detailRows: toolExecutionRows\.detailRows/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /const messageActionSet = createChatMessageActionSet\(\{\s+contentRenderState: messageContentRenderState,\s+turnDuration: \{\s+renderState: messageTurnDurationRenderState,\s+\.\.\.messageActionStyles\.turnDuration,\s+\},\s+speech: \{/);
  ...
  assert.match(chatMessageChromeSource, /const components = createChatMessageActionComponents\(\{\s+availability,\s+\.\.\.input,\s+\}\);/);
  assert.match(chatMessageChromeSource, /const availability = getChatMessageActionAvailabilityRenderState\(\{\s+turnDuration: input\.turnDuration\.renderState\.shouldRender,\s+speech: input\.speech\.renderState\.canSpeak,\s+branch: input\.branch\.renderState\.canBranch,\s+copy: input\.copy\.renderState\.canCopy,\s+expansion: input\.expansion\.renderState\.canToggle,\s+\}\);/);
  ...
  assert.match(screenSource, /body=\{createChatMessageThreadBodyProps\(\{[\s\S]*?conversation: createChatMessageConversationBodyProps\(\{\s+contentState: messageContentRenderState,\s+actionSet: messageActionSet,/);
  ...
  assert.match(screenSource, /isToolOnly: messageDisplayState\.isToolOnly,/);
  assert.doesNotMatch(screenSource, /const messageExpansionRenderState = getChatMessageExpansionMobileRenderState/);
  assert.match(screenSource, /expansion: \{\s+renderState: messageRenderState\.expansion,/);
...
test('keeps the copy action inline with desktop-style message controls', () => {
  ...
  assert.match(screenSource, /copy: \{\s+renderState: messageCopyRenderState,/);
  ...
  assert.match(screenSource, /branch: \{\s+renderState: messageBranchRenderState,[\s\S]*?copy: \{\s+renderState: messageCopyRenderState,[\s\S]*?expansion: \{\s+renderState: messageRenderState\.expansion,/);
...
test('shows shared per-turn duration badges on mobile user messages', () => {
  ...
  assert.match(screenSource, /createChatMessageActionSet\(\{[\s\S]*?turnDuration: \{\s+renderState: messageTurnDurationRenderState,[\s\S]*?speech: \{\s+renderState: messageSpeechRenderState,[\s\S]*?branch: \{\s+renderState: messageBranchRenderState,[\s\S]*?copy: \{\s+renderState: messageCopyRenderState,/);
