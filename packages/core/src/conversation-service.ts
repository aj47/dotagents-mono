...
/**
 * Function signature for content summarization.
 * The conversation service accepts an injectable summarizer so it does not
 * depend directly on the LLM / context-budget module.
 */
export type SummarizeContentFn = (content: string, sessionId?: string) => Promise<string>
...
// Keep low to avoid blocking the main process on large/corrupted files.
...
export class ConversationService {
  ...
  // Injectable summarization function for compaction
  private summarizeFn: SummarizeContentFn | null = null
  ...
  /**
   * Set the summarization function used by the compaction pipeline.
   * Must be called before compaction can work. If not set, compaction is skipped.
   */
  setSummarizeFn(fn: SummarizeContentFn): void {
    this.summarizeFn = fn
  }

  private getConversationsFolder(): string {
    return getConversationsFolder()
  }

  private ensureConversationsFolder() {
    const folder = this.getConversationsFolder()
  ...
  private getConversationPath(conversationId: string): string {
    const folder = this.getConversationsFolder()
    const resolved = path.resolve(folder, `${conversationId}.json`)
  ...
  private getConversationIndexPath(): string {
    return path.join(this.getConversationsFolder(), "index.json")
  }
  ...
  private validateConversationId(conversationId: string): string {
    ...
    const folder = this.getConversationsFolder()
    const resolvedPath = path.resolve(folder, `${sanitized}.json`)
  ...
  private async compactOnLoad(conversation: Conversation, sessionId?: string): Promise<Conversation> {
    ...
    // If no summarization function is set, skip compaction
    if (!this.summarizeFn) {
      logApp(`[ConversationService] compactOnLoad: no summarization function set, skipping compaction`)
      return conversation
    }
    ...
    try {
      summaryContent = await this.summarizeFn(summarizationPrompt, sessionId)
