...
- **Headless CLI agent selection** now also exposes `/agent <agent-id-or-name>`, and that switch shares one activation helper with desktop agent selection plus remote/mobile profile switching so profile-specific model/audio and MCP runtime config apply the same way everywhere.
- **Headless CLI agent profile management** now also exposes `/agents`, `/agent-show <agent-id-or-name>`, `/agent-new <json>`, `/agent-edit <agent-id-or-name> <json>`, `/agent-toggle <agent-id-or-name>`, and `/agent-delete <agent-id-or-name>`, and those commands share the same profile list/detail/create/update/delete/toggle helper path that the desktop Agents settings screen and remote `/v1/agent-profiles` routes now use.
