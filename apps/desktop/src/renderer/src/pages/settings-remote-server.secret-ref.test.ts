...
describe("remote server settings secret references", () => {
  it("uses shared remote server option values", () => {
    ...
    expect(source).toContain("REMOTE_SERVER_API_KEY_FIELD_METADATA")
    expect(source).not.toContain('label="API Key"')
    ...
    expect(source).toContain("CLOUDFLARE_TUNNEL_CREDENTIALS_PATH_FIELD_METADATA")
    expect(source).not.toContain('label="Credentials Path"')
    expect(source).not.toContain('placeholder="~/.cloudflared/<tunnel-id>.json (default)"')
  ...
  it("uses the explicit pairing API key endpoint for copy and QR payloads", () => {
    ...
    expect(source).toContain("!(cfg?.streamerModeEnabled ?? DEFAULT_STREAMER_MODE_ENABLED)")
    expect(source).toContain("REMOTE_SERVER_API_KEY_FIELD_METADATA.secretMask")
