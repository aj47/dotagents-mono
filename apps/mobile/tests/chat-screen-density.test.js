...
test('keeps agent selection in the navigation header for the mobile chat screen', () => {
  ...
  assert.match(screenSource, /agentSelector: \{\s+renderState: mobileHeaderAgentSelectorRenderState,\s+onPress: \(\) => setAgentSelectorVisible\(true\),\s+labelNumberOfLines: mobileHeaderSurface\.agentSelectorText\.numberOfLines,\s+\}/);
...
test('shows the shared total agent time in the mobile chat header', () => {
  ...
  assert.match(screenSource, /turnDuration: \{\s+renderState: headerTotalTurnDurationRenderState,\s+\}/);
...
test('keeps pinning available from the individual chat view header', () => {
  ...
  assert.match(screenSource, /backButton: \{\s+renderState: mobileHeaderBackRenderState,\s+onPress: \(\) => navigation\.navigate\('Sessions'\),\s+\}/);
  assert.match(screenSource, /pinButton: \{\s+renderState: headerPinMobileRenderState,\s+onPress: handleToggleCurrentSessionPinned,\s+isActive: headerPinMobileRenderState\.isPinned,\s+\}/);
...
test('uses shared runtime header copy for mobile stop and hands-free controls', () => {
  ...
  assert.match(screenSource, /handsFreeButton: \{\s+renderState: headerHandsFreeMobileRenderState,\s+onPress: toggleHandsFree,\s+\}/);
...
test('uses shared runtime presentation for the mobile chat viewport and loading state', () => {
  ...
  assert.match(screenSource, /threadList: \{\s+threadStates: conversationThreadStates,\s+styles: chatMessageConversationThreadStyles\.runtimeThread,\s+\}/);
  ...
  assert.match(screenSource, /agentSelector: \{[\s\S]*?visible: agentSelectorVisible,[\s\S]*?onClose: \(\) => setAgentSelectorVisible\(false\),[\s\S]*?\}/);
  assert.match(screenSource, /promptEditor: \{[\s\S]*?visible: addPromptModalVisible,[\s\S]*?onSave: handleSavePrompt,[\s\S]*?\}/);
  ...
  assert.match(chatMessageChromeSource, /overlays: \{\s+agentSelector,\s+promptEditor,\s+\}/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /threadList: \{\s+threadStates: conversationThreadStates,/);
  ...
  assert.match(screenSource, /styles: chatMessageConversationThreadStyles\.runtimeThread,/);
...
test('lets mobile edit and delete desktop saved prompts from quick-start cards', () => {
  ...
  assert.match(screenSource, /const chatMessageRuntimeSurface = createChatMessageRuntimeSurfaceChromeProps\(\{[\s\S]*?promptEditor: \{[\s\S]*?visible: addPromptModalVisible,/);
