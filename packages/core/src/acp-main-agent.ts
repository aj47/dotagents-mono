...
import type { AgentProgressUpdate, AgentProgressStep, SessionProfileSnapshot } from "./types"
import type { ACPConfigOption, ToolCall, ToolResult } from "@dotagents/shared"
...
/**
 * BUILTIN_SERVER_NAME and MARK_WORK_COMPLETE_TOOL defined locally to avoid
 * coupling to desktop's builtin-tool-names module.
 */
const BUILTIN_SERVER_NAME = "dotagents-internal"
const MARK_WORK_COMPLETE_TOOL = "mark_work_complete"
...
/**
 * Set the ProgressEmitter for the ACP main agent.
 */
export function setACPMainAgentProgressEmitter(emitter: ProgressEmitter): void {
  _progressEmitter = emitter
}

/**
 * Set the ConversationService for the ACP main agent.
 */
export function setACPMainAgentConversationService(service: ConversationService): void {
...
/** Helper to get conversation service */
function getConversationService(): ConversationService {
  if (!_conversationService) {
    throw new Error("ConversationService not set for ACP main agent. Call setACPMainAgentConversationService() at startup.")
  }
...
export async function processTranscriptWithACPAgent(
  ...
  try {
    const conversation = await getConversationService().loadConversation(conversationId)
    if (conversation) {
