...
function schedulePostRunAutoGeneratedTitleRefresh(conversationId: string, source: string): void {
  void (async () => {
    try {
      diagnosticsService.logInfo("remote-server", `Refreshing auto-generated title for ${conversationId} after ${source}`)
      const retitledConversation = await conversationService.maybeAutoGenerateConversationTitle(conversationId)
      if (!retitledConversation?.title) {
        diagnosticsService.logInfo("remote-server", `No auto-generated title update for ${conversationId} after ${source}`)
        return
      }

      const updatedSessions = agentSessionTracker.updateConversationTitleForConversation(
        conversationId,
        retitledConversation.title,
      )
      diagnosticsService.logInfo(
        "remote-server",
        `Auto-generated title for ${conversationId}: ${retitledConversation.title} (sessions updated: ${updatedSessions})`,
      )
      notifyConversationHistoryChanged()
    } catch (error) {
      diagnosticsService.logWarning("remote-server", `Failed to refresh auto-generated title for ${conversationId}`, error)
    }
  })()
}
    ...
    try {
      ...
      if (isStreaming) {
        ...
        try {
          ...
          schedulePostRunAutoGeneratedTitleRefresh(result.conversationId, "streaming chat completion")
      ...
      schedulePostRunAutoGeneratedTitleRefresh(result.conversationId, "chat completion")
