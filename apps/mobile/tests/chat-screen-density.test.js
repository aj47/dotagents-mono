...
test('keeps agent selection in the navigation header for the mobile chat screen', () => {
  ...
  assert.match(chatMessageChromeSource, /const headerOptionParts = createChatRuntimeNavigationHeaderOptionsParts\(\{/);
  assert.match(chatMessageChromeSource, /const headerParts = createChatRuntimeNavigationHeaderOptionsMobilePropsParts\(\{\s+\.\.\.headerOptionParts,\s+styles,\s+\}\);/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(chatMessageChromeSource, /const conversationContentParts = createChatRuntimeConversationContentMobilePropsParts\(\{\s+contentDisplayMode,\s+rowStyle,\s+shouldRenderActionSlots,\s+entries,\s+expanded,\s+collapsed,\s+\}\);/);
