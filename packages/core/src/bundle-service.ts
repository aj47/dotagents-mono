...
// Injected UserInteraction for file dialogs
let bundleUserInteraction: UserInteraction | null = null

/**
 * Set the UserInteraction for file dialogs (save/open).
 */
export function setBundleServiceUserInteraction(ui: UserInteraction): void {
  bundleUserInteraction = ui
}
...
async function saveBundleToFile(bundle: DotAgentsBundle): Promise<ExportBundleToFileResult> {
  if (!bundleUserInteraction) {
    return { success: false, filePath: null, canceled: false, error: "No UserInteraction configured. Call setBundleServiceUserInteraction() first." }
  }
  ...
  try {
    ...
    try {
      filePath = await bundleUserInteraction.saveFile({
        title: "Export Agent Configuration",
        defaultName: `${bundle.manifest.name.replace(/[^a-zA-Z0-9-_ ]/g, "")}.dotagents`,
        filters: [
    ...
    try {
      fs.writeFileSync(filePath, bundleJson, "utf-8")
      logApp("[bundle-service] Bundle saved to", { filePath })
      return { success: true, filePath, canceled: false }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      logApp("[bundle-service] Failed to save bundle", { filePath, error })
      return { success: false, filePath: null, canceled: false, error: errorMessage }
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error)
...
export async function previewBundleFromDialog(): Promise<{
  ...
  const filePaths = await bundleUserInteraction.pickFile({
