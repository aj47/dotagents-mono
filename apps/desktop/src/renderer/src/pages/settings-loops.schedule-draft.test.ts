...
describe("settings loops schedule drafts", () => {
  ...
  it("uses shared repeat task execution defaults in the edit form", () => {
    ...
    expect(settingsLoopsSource).toContain("...(editing.profileId ? { profileId: editing.profileId } : {})")
    expect(settingsLoopsSource).toContain("...(parsedMaxIterations ? { maxIterations: parsedMaxIterations } : {})")
