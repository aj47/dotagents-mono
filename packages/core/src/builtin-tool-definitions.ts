...
// The BUILTIN_SERVER_NAME constant is the canonical source.
// Desktop's @shared/builtin-tool-names re-exports from here.
...
// Injected ACP router tool definitions
// Set by the ACP module when it is loaded (or by the desktop app at startup)
let _acpRouterToolDefinitions: BuiltinToolDefinition[] = []

/**
 * Set the ACP router tool definitions.
 * Called by the ACP module or the app at startup.
 */
export function setAcpRouterToolDefinitions(definitions: BuiltinToolDefinition[]): void {
  _acpRouterToolDefinitions = definitions
}

/**
 * Get the current ACP router tool definitions.
 */
export function getAcpRouterToolDefinitions(): BuiltinToolDefinition[] {
  return _acpRouterToolDefinitions
}

// Tool definitions — built-in tools use plain names (no server prefix)
const coreBuiltinToolDefinitions: BuiltinToolDefinition[] = [
  {
...
/**
 * Get the full list of builtin tool definitions, including injected ACP router tools.
 */
export const builtinToolDefinitions: BuiltinToolDefinition[] = new Proxy(coreBuiltinToolDefinitions, {
  get(target, prop, receiver) {
    // When iterating/accessing the array, combine core + ACP definitions
    if (prop === Symbol.iterator || prop === 'length') {
      const combined = [...target, ..._acpRouterToolDefinitions]
    ...
    // Array method access (map, filter, etc.)
    const combined = [...target, ..._acpRouterToolDefinitions]
    const val = (combined as any)[prop]
    if (typeof val === 'function') {
      return val.bind(combined)
    }
...
export function getBuiltinToolNames(): string[] {
  return [...coreBuiltinToolDefinitions, ..._acpRouterToolDefinitions].map((tool) => tool.name)
}
