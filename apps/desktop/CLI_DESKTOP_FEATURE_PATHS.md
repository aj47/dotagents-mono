...
## Shared MCP OAuth management

- Shared MCP OAuth file: `apps/desktop/src/main/mcp-oauth-management.ts`
- Shared helpers: `getManagedMcpOAuthStatus(...)`, `getManagedMcpOAuthServers(...)`, `getManagedMcpOAuthServer(...)`, `resolveManagedMcpOAuthServerSelection(...)`, `startManagedMcpOAuthFlow(...)`, `completeManagedMcpOAuthFlow(...)`, and `revokeManagedMcpOAuthTokens(...)`
- Current callers: `headless-cli.ts` `/mcp-oauth`, `/mcp-oauth-show`, `/mcp-oauth-start`, `/mcp-oauth-complete`, and `/mcp-oauth-revoke`; plus `tipc.ts` `initiateOAuthFlow(...)`, `completeOAuthFlow(...)`, `getOAuthStatus(...)`, and `revokeOAuthTokens(...)` used by `mcp-config-manager.tsx` and `OAuthServerConfig.tsx`
...
60. Headless CLI + desktop MCP OAuth management
    `headless-cli.ts` now routes `/mcp-oauth`, `/mcp-oauth-show <server-name-prefix>`, `/mcp-oauth-start <server-name-prefix>`, `/mcp-oauth-complete <server-name-prefix> <code> <state>`, and `/mcp-oauth-revoke <server-name-prefix>` through `mcp-oauth-management.ts`, while `tipc.ts` reuses the same helper for `mcp-config-manager.tsx` and `OAuthServerConfig.tsx`, so streamable HTTP server selection, auth-status inspection, headless-safe flow initiation, manual code/state completion, and token revocation stay aligned across the terminal and desktop capabilities UI.
61. Headless CLI + desktop model catalog management
...
- MCP OAuth status inspection and auth lifecycle actions are decided in one place: `getManagedMcpOAuthStatus(...)`, `getManagedMcpOAuthServers(...)`, `getManagedMcpOAuthServer(...)`, `resolveManagedMcpOAuthServerSelection(...)`, `startManagedMcpOAuthFlow(...)`, `completeManagedMcpOAuthFlow(...)`, and `revokeManagedMcpOAuthTokens(...)`, so headless CLI `/mcp-oauth`, `/mcp-oauth-show`, `/mcp-oauth-start`, `/mcp-oauth-complete`, and `/mcp-oauth-revoke` plus the desktop capabilities UI all reuse the same streamable HTTP filtering, selection, and auth-token lifecycle behavior before presentation diverges into terminal output or desktop controls.
...
- `apps/desktop/src/main/mcp-oauth-management.test.ts`
  Confirms shared MCP OAuth server filtering, exact/prefix selection, headless-safe flow start behavior, manual completion, and token revocation stay aligned for headless CLI and desktop capabilities callers.
