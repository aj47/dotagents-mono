...
import { configStore, getRecordingsFolder } from "./config"
...
import type { AgentProgressUpdate, SessionProfileSnapshot, LoopConfig } from "./types"
...
// Injected ProgressEmitter for notifying UI about changes
let progressEmitter: ProgressEmitter | null = null

/**
 * Set the ProgressEmitter for sending UI notifications.
 */
export function setRemoteServerProgressEmitter(emitter: ProgressEmitter): void {
  progressEmitter = emitter
}

// Injected ConversationService
let _conversationService: ConversationService | null = null

/**
 * Set the ConversationService for the remote server.
 */
export function setRemoteServerConversationService(service: ConversationService): void {
  _conversationService = service
}

function getConversationService(): ConversationService {
  if (!_conversationService) {
    throw new Error("ConversationService not set. Call setRemoteServerConversationService() at startup.")
  }
  return _conversationService
}

// Alias for backward compat in this file
// Helper to access the conversation service with methods exposed
function get_cs(): ConversationService {
...
/**
 * Notify the UI that conversation history has changed.
 * Used after remote server creates or modifies conversations (e.g. from mobile).
 * Uses ProgressEmitter abstraction instead of direct WINDOWS/tipc access.
 */
function notifyConversationHistoryChanged(): void {
  if (progressEmitter) {
    try {
      // Emit a generic event indicating conversation history was modified
      // The platform-specific emitter will translate this into the appropriate
      // notification mechanism (e.g., tipc broadcast for Electron, state update for CLI)
      progressEmitter.emitConversationHistoryChanged?.()
    } catch (err) {
      diagnosticsService.logWarning("remote-server", `Failed to notify about conversation history changes: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
}
...
async function runAgent(options: RunAgentOptions): Promise<{
  ...
  if (conversationId) {
    ...
    const updatedConversation = await get_cs().addMessageToConversation(
      conversationId,
      prompt,
      "user"
    )
    ...
    if (updatedConversation) {
      ...
      const newConversation = await get_cs().createConversationWithId(conversationId, prompt, "user")
  ...
  if (!conversationId) {
    const newConversation = await get_cs().createConversationWithId(
      get_cs().generateConversationIdPublic(),
      prompt,
      "user"
    )
    conversationId = newConversation.id
  ...
  const loadFormattedConversationHistory = async () => {
    const latestConversation = await get_cs().loadConversation(conversationId)
  ...
  try {
    if (cfg.mainAgentMode === "acp" && cfg.mainAgentName) {
      ...
      try {
        ...
        if (result.response) {
          await get_cs().addMessageToConversation(conversationId, result.response, "assistant")
        }
...
function recordHistory(transcript: string) {
  try {
    ...
    const historyPath = path.join(recFolder, "history.json")
...
async function startRemoteServerInternal(options: StartRemoteServerOptions = {}) {
  ...
  fastify.get("/v1/settings", async (_req, reply) => {
    try {
      ...
      const mergedPresets = builtInPresets.map(builtIn => {
        const savedOverride = savedPresets.find((p: any) => p.id === builtIn.id)
        if (savedOverride) {
      ...
      const customPresets = savedPresets.filter((p: any) => !builtInIds.has(p.id))
      ...
      return reply.send({
        ...
        predefinedPrompts: (cfg.predefinedPrompts || []).map((prompt: any) => ({
          id: prompt.id,
          name: prompt.name,
  ...
  fastify.patch("/v1/settings", async (req, reply) => {
    try {
      ...
      if (typeof body.currentModelPresetId === "string") {
        ...
        const savedPresets = cfg.modelPresets || []
        const builtInIds = new Set(builtInPresets.map(p => p.id))
        const allValidIds = new Set([...builtInIds, ...savedPresets.filter((p: any) => !builtInIds.has(p.id)).map((p: any) => p.id)])
  ...
  fastify.get("/v1/conversations/:id", async (req, reply) => {
    try {
      ...
      const conversation = await get_cs().loadConversation(conversationId)
  ...
  fastify.post("/v1/push/register", async (req, reply) => {
    try {
      ...
      const existingIndex = existingTokens.findIndex((t: any) => t.token === body.token)
      const newToken = {
        token: body.token,
        type: "expo" as const,
  ...
  fastify.post("/v1/push/unregister", async (req, reply) => {
    try {
      ...
      const filteredTokens = existingTokens.filter((t: any) => t.token !== body.token)
  ...
  fastify.get("/v1/push/status", async (_req, reply) => {
    try {
      ...
      return reply.send({
        ...
        platforms: [...new Set(tokens.map((t: any) => t.platform))],
      })
    } catch (error: any) {
  ...
  fastify.get("/v1/conversations", async (_req, reply) => {
    try {
      const conversations = await get_cs().getConversationHistory()
  ...
  fastify.post("/v1/conversations", async (req, reply) => {
    try {
      ...
      const conversationId = get_cs().generateConversationIdPublic()
      const now = Date.now()
      ...
      await get_cs().saveConversation(conversation, true)
  ...
  fastify.put("/v1/conversations/:id", async (req, reply) => {
    try {
      ...
      const now = Date.now()
      let conversation = await get_cs().loadConversation(conversationId)
  ...
  fastify.post("/v1/agent-profiles", async (req, reply) => {
    try {
      ...
      const connection: import("./types").AgentProfileConnection = sanitizeAgentProfileConnection({
  ...
  fastify.patch("/v1/agent-profiles/:id", async (req, reply) => {
    try {
      ...
      const updates: Partial<import("./types").AgentProfile> = {}
  ...
  fastify.post("/v1/loops/:id/toggle", async (req, reply) => {
    try {
      ...
      const loopIndex = loops.findIndex((l: any) => l.id === params.id)
  ...
  fastify.delete("/v1/loops/:id", async (req, reply) => {
    try {
      ...
      const updatedLoops = loops.filter((l: any) => l.id !== params.id)
      configStore.save({ ...cfg, loops: updatedLoops })
