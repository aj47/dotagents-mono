...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /accessibilityLabel: messageRenderState\.expansion\.accessibilityLabel/);
...
test('uses shared desktop chat message presentation tones for mobile message cards', () => {
  ...
  assert.match(screenSource, /collapsed: \{\s+renderState: messageRenderState\.collapsedPreview,[\s\S]*?onPress: messageActionAvailabilityRenderState\.expansion\.canRender\s+\? \(\) => toggleMessageExpansion\(i\)\s+: undefined,[\s\S]*?accessibilityLabel: messageRenderState\.expansion\.accessibilityLabel,/);
  ...
  assert.match(screenSource, /accessibilityState: messageRenderState\.expansion\.accessibilityState/);
  assert.equal((screenSource.match(/ariaExpanded=\{messageRenderState\.expansion\.ariaExpanded\}/g) ?? []).length, 0);
  assert.match(screenSource, /ariaExpanded: messageRenderState\.expansion\.ariaExpanded/);
