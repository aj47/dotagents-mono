...
test('keeps agent selection in the navigation header for the mobile chat screen', () => {
  ...
  assert.match(screenSource, /const currentAgentLabel = getChatRuntimeCurrentAgentLabel\(currentProfile\?\.name\);/);
  ...
  assert.match(screenSource, /const mobileHeaderAgentSelectorRenderState = useMemo\(\s+\(\) => getChatRuntimeAgentSelectorMobileRenderState\(\{\s+agentLabel: currentAgentLabel,\s+colors: theme\.colors,\s+\}\),\s+\[currentAgentLabel, theme\.colors\],\s+\);/);
  assert.match(screenSource, /agentSelectorRenderState: mobileHeaderAgentSelectorRenderState,\s+onAgentSelectorPress: \(\) => setAgentSelectorVisible\(true\),\s+agentSelectorLabelNumberOfLines: mobileHeaderSurface\.agentSelectorText\.numberOfLines,/);
  ...
  assert.match(screenSource, /const mobileHeaderSurface = getChatRuntimeHeaderMobileSurfaceState\(\);/);
...
test('shows a conversation-state chip in the mobile chat header while preserving the compact header actions row', () => {
  ...
  assert.match(screenSource, /conversationStatusRenderState: headerConversationStatus,\s+conversationStatusSpinnerSource: isDark \? darkSpinner : lightSpinner,/);
...
test('shows the shared total agent time in the mobile chat header', () => {
  ...
  assert.match(screenSource, /turnDurationRenderState: headerTotalTurnDurationRenderState,/);
...
test('lets mobile respond to desktop tool approval requests from progress updates', () => {
  ...
  assert.match(screenSource, /getChatRuntimeToolApprovalMobileRenderState,/);
  ...
  assert.match(screenSource, /const toolApprovalStyleState = getChatRuntimeToolApprovalMobileRenderState\(\{\s+toolName: '',\s+colors: theme\.colors,\s+\}\);/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?expandedToolApprovals,[\s\S]*?pendingApprovalResponseId: pendingToolApprovalResponseId,[\s\S]*?onToggleToolApprovalArguments: toggleToolApprovalArguments,[\s\S]*?onRespondToToolApproval: respondToToolApproval,/);
...
test('shows desktop-style retry status updates from shared runtime presentation', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?messages: visibleMessages,[\s\S]*?colors: theme\.colors,/);
  ...
  assert.match(screenSource, /const retryStatusStyleState = getChatRuntimeRetryStatusMobileRenderState\(\{\s*colors: theme\.colors,\s*\}\);/);
...
test('renders delegated agent progress as compact desktop-style mobile chrome', () => {
  ...
  assert.match(screenSource, /const chatMessageConversationThreadPresentation = useMemo\(\s+\(\) => createChatMessageConversationThreadPresentationState\(\{\s+colors: theme\.colors,\s+\}\),\s+\[theme\.colors\],\s+\);/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?allMessages: messages,[\s\S]*?messages: visibleMessages,[\s\S]*?presentation: chatMessageConversationThreadPresentation,/);
...
test('keeps pinning available from the individual chat view header', () => {
  assert.match(screenSource, /getChatRuntimePinMobileRenderState,/);
  assert.match(screenSource, /getChatRuntimeBackMobileRenderState,/);
  assert.match(screenSource, /getChatRuntimeAgentSelectorMobileRenderState,/);
  ...
  assert.match(screenSource, /const mobileHeaderBackRenderState = useMemo\(\s*\(\) => getChatRuntimeBackMobileRenderState\(\{ colors: theme\.colors \}\),\s*\[theme\.colors\],\s*\);/);
  assert.match(screenSource, /const headerPinMobileRenderState = useMemo\(\s+\(\) => getChatRuntimePinMobileRenderState\(\{\s+isPinned: isCurrentSessionPinned,\s+colors: theme\.colors,\s+\}\),\s+\[isCurrentSessionPinned, theme\.colors\],\s+\);/);
  assert.match(screenSource, /backButtonRenderState: mobileHeaderBackRenderState,\s+onBackButtonPress: \(\) => navigation\.navigate\('Sessions'\),/);
  assert.match(screenSource, /pinButtonRenderState: headerPinMobileRenderState,\s+onPinButtonPress: handleToggleCurrentSessionPinned,\s+pinButtonIsActive: headerPinMobileRenderState\.isPinned,/);
...
test('uses shared runtime header copy for mobile stop and hands-free controls', () => {
  ...
  assert.match(screenSource, /killSwitchButtonShouldRender: mobileHeaderKillSwitchVisibilityRenderState\.shouldRender,\s+killSwitchButtonRenderState: mobileHeaderKillSwitchRenderState,\s+onKillSwitchButtonPress: handleKillSwitch,/);
  ...
  assert.match(screenSource, /`\$\{mobileRuntimeKillSwitchAlerts\.connectionFailed\.title\}: \$\{getChatRuntimeAlertMessage\(e, mobileRuntimeKillSwitchAlerts\.connectionFailed\.fallbackMessage\)\}`/);
  assert.match(screenSource, /Alert\.alert\(\s*mobileRuntimeKillSwitchAlerts\.confirmation\.title,\s*mobileRuntimeKillSwitchAlerts\.confirmation\.message,/);
  assert.match(screenSource, /\{ text: mobileRuntimeKillSwitchAlerts\.confirmation\.cancelLabel, style: 'cancel' \}/);
  assert.match(screenSource, /text: mobileRuntimeKillSwitchAlerts\.confirmation\.confirmLabel,/);
  assert.match(screenSource, /Alert\.alert\(\s*mobileRuntimeKillSwitchAlerts\.success\.title,\s*result\.message \|\| mobileRuntimeKillSwitchAlerts\.success\.fallbackMessage,/);
  assert.match(screenSource, /Alert\.alert\(\s*mobileRuntimeKillSwitchAlerts\.failed\.title,\s*result\.error \|\| mobileRuntimeKillSwitchAlerts\.failed\.fallbackMessage,/);
  assert.match(screenSource, /Alert\.alert\(\s*mobileRuntimeKillSwitchAlerts\.connectionFailed\.title,\s*getChatRuntimeAlertMessage\(e, mobileRuntimeKillSwitchAlerts\.connectionFailed\.fallbackMessage\),/);
  ...
  assert.match(screenSource, /handsFreeButtonRenderState: headerHandsFreeMobileRenderState,\s+onHandsFreeButtonPress: toggleHandsFree,/);
  ...
  assert.match(screenSource, /const headerHandsFreeMobileRenderState = useMemo\(\s+\(\) => getChatRuntimeHandsFreeMobileRenderState\(\{\s+isEnabled: handsFree,\s+colors: theme\.colors,\s+\}\),\s+\[handsFree, theme\.colors\],\s+\);/);
  assert.match(screenSource, /const mobileHeaderKillSwitchRenderState = useMemo\(\s+\(\) => getChatRuntimeKillSwitchMobileRenderState\(\{ colors: theme\.colors \}\),\s+\[theme\.colors\],\s+\);/);
...
test('uses shared runtime presentation for mobile scroll-to-bottom affordance', () => {
  ...
  assert.match(screenSource, /getChatRuntimeScrollToBottomMobileRenderState,/);
...
test('uses shared runtime presentation for the mobile chat viewport and loading state', () => {
  ...
  assert.match(screenSource, /const chatMessageRuntimeSurface = createChatMessageRuntimeSurfaceChromeProps\(\{\s+keyboardAvoidingBehavior: mobileRuntimeViewportKeyboardAvoidingBehavior,\s+keyboardVerticalOffset: headerHeight,/);
  ...
  assert.match(screenSource, /visibleMessageCount: visibleMessages\.length,\s+totalMessageCount: messages\.length,\s+hiddenMessageCount,\s+messageHistoryLoadIncrement: CHAT_MESSAGE_HISTORY_WINDOW\.loadIncrement,\s+latestStepSummary,\s+colors: theme\.colors,\s+onLoadEarlierMessages: handleLoadEarlierMessages,/);
...
test('uses shared runtime presentation for mobile connection and retry banners', () => {
  ...
  assert.match(screenSource, /const connectionBannerStyleState = getChatRuntimeConnectionBannerMobileRenderState\(\{\s*colors: theme\.colors,\s*\}\);/);
...
test('uses shared desktop-style icons for mobile composer controls', () => {
  ...
  assert.match(screenSource, /getChatComposerMobileVisibilityRenderState,/);
  ...
  assert.match(screenSource, /const mobileComposerMicRenderState = useMemo\(\s+\(\) => getChatComposerMicMobileRenderState\(\{\s+label: micButtonLabel,\s+handsFree,\s+listening,\s+willCancel,\s+colors: theme\.colors,\s+\}\),\s+\[handsFree, listening, micButtonLabel, theme\.colors, willCancel\],\s+\);/);
  assert.match(screenSource, /const mobileComposerVisibilityRenderState = useMemo\(\s+\(\) => getChatComposerMobileVisibilityRenderState\(\{\s+handsFree,\s+listening,\s+messageQueueEnabled,\s+\}\),\s+\[handsFree, listening, messageQueueEnabled\],\s+\);/);
  ...
  assert.match(screenSource, /imageAttachmentRenderState: mobileComposerImageAttachmentRenderState,\s+onImageAttachmentPress: handlePickImages,/);
  assert.match(screenSource, /textToSpeechRenderState: mobileComposerTextToSpeechRenderState,\s+onTextToSpeechPress: toggleTts,/);
  assert.match(screenSource, /editBeforeSendShouldRender: mobileComposerVisibilityRenderState\.editBeforeSendControl\.shouldRender,\s+editBeforeSendRenderState: mobileComposerEditBeforeSendRenderState,\s+onEditBeforeSendPress: \(\) => setWillCancel\(\(current\) => !current\),/);
  assert.match(chatMessageChromeSource, /imageAttachmentControl: \{\s+renderState: imageAttachmentRenderState,\s+onPress: onImageAttachmentPress,\s+\.\.\.chrome\.imageAttachmentControl,/);
  assert.match(chatMessageChromeSource, /textToSpeechControl: \{\s+renderState: textToSpeechRenderState,\s+onPress: onTextToSpeechPress,\s+\.\.\.chrome\.textToSpeechControl,/);
  assert.match(chatMessageChromeSource, /editBeforeSendControl: \{\s+shouldRender: editBeforeSendShouldRender,\s+renderState: editBeforeSendRenderState,\s+onPress: onEditBeforeSendPress,\s+\.\.\.chrome\.editBeforeSendControl,/);
  ...
  assert.match(chatMessageChromeSource, /queueAction: \{\s+shouldRender: queueActionShouldRender,\s+renderState: queueActionRenderState,\s+onPress: onQueueActionPress,\s+\.\.\.chrome\.queueAction,/);
  ...
  assert.match(screenSource, /setDebugInfo\(mobileComposerQueueRenderState\.debugMessage\)/);
  ...
  assert.match(screenSource, /submitActionRenderState: composerSubmitRenderState,\s+onSubmitActionPress: sendComposerInput,/);
  assert.match(chatMessageChromeSource, /submitAction: \{\s+renderState: submitActionRenderState,\s+onPress: onSubmitActionPress,\s+\.\.\.chrome\.submitAction,/);
  ...
  assert.match(screenSource, /micButtonRenderState: mobileComposerMicRenderState,\s+shouldUsePushToTalk: mobileComposerVisibilityRenderState\.micButton\.shouldUsePushToTalk,\s+shouldUseHandsFreePrimaryControl: mobileComposerVisibilityRenderState\.micButton\.shouldUseHandsFreePrimaryControl,\s+onMicPressIn: handlePushToTalkPressIn,\s+onMicPressOut: handlePushToTalkPressOut,\s+onMicPress: handleHandsFreePrimaryControl,/);
  assert.match(chatMessageChromeSource, /micButton: \{\s+renderState: micButtonRenderState,\s+onPressIn: shouldUsePushToTalk \? onMicPressIn : undefined,\s+onPressOut: shouldUsePushToTalk \? onMicPressOut : undefined,\s+onPress: shouldUseHandsFreePrimaryControl \? onMicPress : undefined,\s+\.\.\.chrome\.micButton,/);
  ...
  assert.match(screenSource, /textEntryInputRef: inputRef,\s+textEntryValue: input,\s+onTextEntryChangeText: handleInputChange,\s+onTextEntryKeyPress: handleInputKeyPress,\s+textEntryAccessibilityLabel: mobileComposerControls\.field\.accessibilityLabel,\s+textEntryAccessibilityHint: composerAccessibilityHint,\s+textEntryPlaceholder: composerPlaceholder,\s+textEntryVoiceStatusLiveRegionAnnouncement: voiceInputLiveRegionAnnouncement,/);
  ...
  assert.match(screenSource, /textEntryVoiceStatusLiveRegionAnnouncement: voiceInputLiveRegionAnnouncement/);
...
test('keeps the live voice overlay compact by grouping status and transcript into one card', () => {
  ...
  assert.match(screenSource, /voiceOverlayVisible: mobileComposerVisibilityRenderState\.voiceOverlay\.isVisible,\s+voiceOverlayLabel,\s+voiceOverlayTranscript: liveTranscript,\s+voiceOverlayTranscriptNumberOfLines: mobileComposerSurface\.voiceOverlay\.transcriptNumberOfLines,/);
  assert.match(chatMessageChromeSource, /voiceOverlay: \{\s+isVisible: voiceOverlayVisible,\s+label: voiceOverlayLabel,\s+transcript: voiceOverlayTranscript,\s+transcriptNumberOfLines: voiceOverlayTranscriptNumberOfLines,\s+\}/);
...
test('caps live transcript height so the recording overlay is less likely to cover the chat surface', () => {
  assert.match(screenSource, /voiceOverlayTranscriptNumberOfLines: mobileComposerSurface\.voiceOverlay\.transcriptNumberOfLines/);
...
test('derives visible assistant content from respond_to_user output and suppresses raw tool payloads', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{\s+allMessages: messages,\s+messages: visibleMessages,\s+firstMessageIndex: firstVisibleMessageIndex,\s+groupByIndex: toolActivityGroups\.groupByIndex,\s+groupState: expandedGroups,\s+inheritedState: expandedMessages,\s+onToggleGroup: toggleGroupExpansion,/);
...
test('uses shared media sanitization for collapsed mobile message previews', () => {
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?allMessages: messages,[\s\S]*?messages: visibleMessages,/);
...
test('uses shared runtime activity copy for mobile loading and thinking states', () => {
  ...
  assert.match(screenSource, /visibleMessageCount: visibleMessages\.length,\s+totalMessageCount: messages\.length,\s+hiddenMessageCount,\s+messageHistoryLoadIncrement: CHAT_MESSAGE_HISTORY_WINDOW\.loadIncrement,\s+latestStepSummary,\s+colors: theme\.colors,/);
  ...
  assert.match(screenSource, /createChatMessageConversationThreadListRenderState\(\{\s+allMessages: messages,\s+messages: visibleMessages,\s+firstMessageIndex: firstVisibleMessageIndex,\s+groupByIndex: toolActivityGroups\.groupByIndex,\s+groupState: expandedGroups,\s+inheritedState: expandedMessages,\s+onToggleGroup: toggleGroupExpansion,/);
...
test('uses desktop-style streaming response chrome while mobile assistant content is live', () => {
  assert.match(screenSource, /getChatRuntimeStreamingContentMobileRenderState,/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?allMessages: messages,[\s\S]*?messages: visibleMessages,[\s\S]*?spinnerSource: isDark \? darkSpinner : lightSpinner,[\s\S]*?onToggleMessageExpansion: toggleMessageExpansion,/);
  ...
  assert.match(screenSource, /const streamingContentStyleState = getChatRuntimeStreamingContentMobileRenderState\(\{\s+colors: theme\.colors,\s+\}\);/);
...
test('surfaces desktop step summaries as compact mobile runtime chrome without persisting them as messages', () => {
  ...
  assert.match(screenSource, /const stepSummaryStyleState = getChatRuntimeStepSummaryMobileRenderState\(\{\s+colors: theme\.colors,\s+\}\);/);
...
test('uses shared message queue surface tokens for the chat-adjacent queue wrapper', () => {
  ...
  assert.match(screenSource, /queuePanelShouldRender: messageQueuePanelDockRenderState\.shouldRender,\s+queuePanelConversationId: currentConversationId,\s+queuedMessages,/);
  assert.match(chatMessageChromeSource, /queuePanel: \{\s+shouldRender: queuePanelShouldRender,\s+panel: \{\s+conversationId: queuePanelConversationId,\s+messages: queuedMessages,/);
...
test('uses tool activities wording consistently for grouped tool activity labels', () => {
  ...
  assert.match(screenSource, /createChatMessageConversationThreadListRenderState,/);
  ...
  assert.match(screenSource, /applyChatDisplayGroupedExpansionInheritance,/);
  assert.match(screenSource, /setExpandedGroups\(prev => applyChatDisplayGroupedExpansionInheritance\(\{/);
  assert.match(screenSource, /inheritedState: expandedMessages,/);
  assert.match(screenSource, /groups: getToolActivityGroupExpansionInheritanceItems\(toolActivityGroups\.groups\),/);
  assert.match(screenSource, /const key = getToolActivityGroupStateKey\(group\);/);
...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(screenSource, /import \{[\s\S]*?ChatMessageRuntimeSurface,[\s\S]*?createChatMessageConversationThreadListRenderState,[\s\S]*?createChatMessageConversationThreadPresentationState,[\s\S]*?createChatMessageConversationThreadStyleSlots,[\s\S]*?\} from '\.\.\/ui\/ChatMessageChrome';/);
  assert.doesNotMatch(screenSource, /ChatMessageConversationRuntimeThreadList,/);
  ...
  assert.doesNotMatch(screenSource, /createChatMessageConversationRenderContext,/);
  assert.match(screenSource, /createChatMessageConversationThreadListRenderState,/);
  assert.match(screenSource, /createChatMessageConversationThreadPresentationState,/);
  ...
  assert.match(screenSource, /const conversationThreadStates = createChatMessageConversationThreadListRenderState\(\{[\s\S]*?allMessages: messages,[\s\S]*?messages: visibleMessages,[\s\S]*?turnDurationsByUserTimestamp: turnDurations\.byUserTimestamp,/);
...
test('shows shared per-turn duration badges on mobile user messages', () => {
  ...
  assert.match(screenSource, /const turnDurationMessages = useMemo<TurnDurationMessage\[\]>/);
...
test('uses shared runtime presentation for mobile request and queue debug copy', () => {
  ...
  assert.match(screenSource, /mobileRuntimeDebug\.requestSent/);
  assert.match(screenSource, /mobileRuntimeDebug\.completed/);
  assert.match(screenSource, /mobileRuntimeDebug\.processingQueuedMessage/);
  assert.match(screenSource, /mobileRuntimeDebug\.sessionChangedDuringProcessing/);
  assert.match(screenSource, /mobileRuntimeDebug\.requestSuperseded/);
  assert.match(screenSource, /mobileRuntimeDebug\.unknownError/);
