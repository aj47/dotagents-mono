...
import type { SystemPromptAdditions } from "./system-prompts"
...
/**
 * MARK_WORK_COMPLETE_TOOL is a desktop-app-specific tool name constant.
 * Defined locally to avoid coupling core to desktop's builtin-tool-names module.
 */
export const MARK_WORK_COMPLETE_TOOL = "mark_work_complete"

/**
 * Internal completion nudge: injected into LLM context, hidden from progress UI.
 */
...
/**
 * Minimal interface for the memory service methods used by llm.ts.
 */
...
/**
 * Minimal interface for the agent session tracker methods used by llm.ts.
 */
export interface LLMAgentSessionTracker {
  isSessionSnoozed(sessionId: string): boolean
  getSession(sessionId: string): { profileSnapshot?: SessionProfileSnapshot; conversationTitle?: string } | undefined
}

/**
 * Minimal interface for the skills service methods used by llm.ts.
 */
export interface LLMSkillsService {
  getSkills(): Array<{ id: string }>
  getEnabledSkillsInstructionsForProfile(enabledSkillIds: string[]): string | undefined
}

/**
 * Minimal interface for the agent profile service methods used by llm.ts.
 */
export interface LLMAgentProfileService {
...
let _memoryService: LLMMemoryService | null = null
let _agentSessionTracker: LLMAgentSessionTracker | null = null
let _conversationService: ConversationService | null = null
let _skillsService: LLMSkillsService | null = null
let _agentProfileService: LLMAgentProfileService | null = null
/** Optional callback to compute system prompt additions (ACP routing, agents, sub-sessions). */
let _systemPromptAdditionsFn: ((excludeAgentId?: string) => SystemPromptAdditions) | null = null

/**
 * Set the ProgressEmitter implementation. Must be called at startup.
 */
export function setLLMProgressEmitter(emitter: ProgressEmitter): void {
  _progressEmitter = emitter
}

/**
 * Set the memory service implementation. Must be called at startup.
 */
export function setLLMMemoryService(service: LLMMemoryService): void {
  _memoryService = service
}

/**
 * Set the agent session tracker implementation. Must be called at startup.
 */
export function setLLMAgentSessionTracker(tracker: LLMAgentSessionTracker): void {
  _agentSessionTracker = tracker
}

/**
 * Set the conversation service instance. Must be called at startup.
 */
export function setLLMConversationService(service: ConversationService): void {
  _conversationService = service
}

/**
 * Set the skills service implementation. Must be called at startup.
 */
export function setLLMSkillsService(service: LLMSkillsService): void {
  _skillsService = service
}

/**
 * Set the agent profile service implementation. Must be called at startup.
 */
export function setLLMAgentProfileService(service: LLMAgentProfileService): void {
  _agentProfileService = service
}

/**
 * Set the system prompt additions callback. Called during constructSystemPrompt
 * to obtain ACP routing prompt, agents delegation prompt, and sub-session prompt.
 */
export function setSystemPromptAdditionsFn(fn: (excludeAgentId?: string) => SystemPromptAdditions): void {
  _systemPromptAdditionsFn = fn
}

/** Helper to get injected progress emitter or no-op */
...
/** Helper to get the conversation service (throws if not set) */
function getConversationService(): ConversationService {
  if (!_conversationService) {
    throw new Error("ConversationService not set. Call setLLMConversationService() at startup.")
  }
  return _conversationService
}

/** Lazy accessor helpers for optional services */
const memoryService = {
  async getAllMemories(): Promise<AgentMemory[]> {
    return _memoryService?.getAllMemories() ?? []
  },
  createMemoryFromSummary(
    summary: AgentStepSummary,
    title?: string,
    ...
    return _memoryService?.createMemoryFromSummary(summary, title, userNotes, tags, conversationTitle, conversationId) ?? null
  },
  saveMemory(memory: AgentMemory): Promise<void> {
    return _memoryService?.saveMemory(memory) ?? Promise.resolve()
  },
}

const agentSessionTracker = {
  isSessionSnoozed(sessionId: string): boolean {
    return _agentSessionTracker?.isSessionSnoozed(sessionId) ?? false
  },
  getSession(sessionId: string): { profileSnapshot?: SessionProfileSnapshot; conversationTitle?: string } | undefined {
    return _agentSessionTracker?.getSession(sessionId)
  },
}
...
export async function processTranscriptWithTools(
  ...
  const mainAgent = _agentProfileService?.getCurrentProfile()
  const userGuidelines = mainAgent?.guidelines || ""
  const enabledSkillIdsOrNull = mainAgent && _agentProfileService
    ? _agentProfileService.getEnabledSkillIdsForProfile(mainAgent.id)
    : []
  ...
  const enabledSkillIds = enabledSkillIdsOrNull === null
    ? (_skillsService?.getSkills().map(s => s.id) ?? [])
    : enabledSkillIdsOrNull
  const skillsInstructions = _skillsService?.getEnabledSkillsInstructionsForProfile(enabledSkillIds)
...
export async function processTranscriptWithAgentMode(
  ...
  const stepSummaries: import('@dotagents/shared').AgentStepSummary[] = []
  ...
  const saveMessageIncremental = async (
    ...
    try {
      ...
      await getConversationService().addMessageToConversation(
  ...
  if (!agentSkillsInstructions) {
    ...
    profileSkillsInstructions = _skillsService?.getEnabledSkillsInstructionsForProfile(enabledSkillIds)
  ...
  // Compute system prompt additions (ACP, agents, sub-sessions) if the host app provided a callback
  const systemPromptAdditions = _systemPromptAdditionsFn?.(excludeAgentId)
  ...
  const systemPrompt = constructSystemPrompt(
    ...
    systemPromptAdditions, // ACP routing, agents delegation, sub-session instructions
  )
