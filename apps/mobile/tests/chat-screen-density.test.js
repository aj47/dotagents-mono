...
test('keeps the TTS control inline with assistant message text instead of on a detached row', () => {
  ...
  assert.match(chatMessageChromeSource, /createChatMessageActionSlotRenderState,/);
  assert.match(chatMessageChromeSource, /const actionRenderState = createChatMessageActionSlotRenderState<ReactNode>\(\{\s+availability,\s+renderState: contentRenderState,\s+renderers: createChatMessageActionRenderers\(actionInput\),\s+\}\);/);
  assert.match(chatMessageChromeSource, /shouldRenderActionSlots: actionRenderState\.shouldRenderActionSlots,/);
