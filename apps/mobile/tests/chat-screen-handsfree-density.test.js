...
test('renders the extracted handsfree status chip in the mobile chat composer', () => {
  ...
  assert.match(sessionPresentationSource, /isVisible: controlRenderState\.visibility\.handsFreeControls\.isVisible,/);
  assert.match(chatMessageChromeSource, /import \{ HandsFreeStatusChip \} from '\.\/HandsFreeStatusChip';/);
  assert.match(sessionPresentationSource, /const handsFreeControlsRenderState = getChatComposerRuntimeHandsFreeControlsMobileRenderState\(\{[\s\S]*?phase: handsFreeStatusPhase,[\s\S]*?label: handsFreeStatusLabel,[\s\S]*?isEnabled: handsFreeStatusEnabled,[\s\S]*?wakePhrase: handsFreeStatusWakePhrase,[\s\S]*?sleepPhrase: handsFreeStatusSleepPhrase,[\s\S]*?lastError: handsFreeStatusLastError,[\s\S]*?foregroundOnly: handsFreeStatusForegroundOnly,[\s\S]*?\}\)/);
  assert.match(sessionPresentationSource, /status: handsFreeControlsRenderState\.status,/);
  assert.match(sessionPresentationSource, /export function getChatComposerRuntimeHandsFreeControlsMobileRenderState/);
  assert.match(sessionPresentationSource, /subtitle: isEnabled\s+\? getHandsFreeStatusSubtitle\(\{/);
  ...
  assert.match(chatMessageChromeSource, /<ChatComposerHandsFreeStatusRow\s+\{\.\.\.handsFreeStatusRow\.props\}/);
  assert.match(chatMessageChromeSource, /<ChatComposerHandsFreeStatusRowContent\s+\{\.\.\.handsFreeStatusRow\.content\}/);
  assert.match(chatMessageChromeSource, /<ChatComposerHandsFreeControlsRow\s+\{\.\.\.handsFreeControlsRow\.props\}/);
  assert.match(chatMessageChromeSource, /<ChatComposerHandsFreeControlsRowContent\s+\{\.\.\.handsFreeControlsRow\.content\}/);
  ...
  assert.match(screenSource, /const \{ chatRuntimeChrome \} = useChatRuntimeMobileStyleSlots\(\);/);
  assert.match(chatRuntimeMobileStylesSource, /handsFreeStatusRow:\s*\{\s+\.\.\.handsFreeStyleSlots\.statusRow,\s+\}/);
  assert.match(sessionPresentationSource, /paddingHorizontal:\s*spacing\[surface\.statusRow\.paddingHorizontal\]/);
...
test('lets handsfree users queue a drafted message without sending immediately', () => {
  ...
  assert.match(sessionPresentationSource, /text: renderState\.label/);
});
...
test('derives send-next availability from strict FIFO queue semantics', () => {
  ...
  assert.match(sessionPresentationSource, /canProcessNext: canProcessNextQueuedMessage/);
});
...
test('wires ChatScreen through the extracted handsfree controller and recognizer hooks', () => {
  ...
  assert.match(screenSource, /createChatRuntimeMobileConfigState,/);
...
test('resets the handsfree controller before shutting down recognizer state when toggled off', () => {
  ...
  assert.match(chatMessageChromeSource, /if \(!next\) \{[\s\S]*?handsFreeController\.reset\(\);[\s\S]*?void stopRecognitionOnly\(\);[\s\S]*?stopSpeech\(\);[\s\S]*?stopRemoteSpeech\(\);[\s\S]*?getChatComposerRuntimeHandsFreeDebugMessage\('disabled'\)/);
...
test('keeps wake/sleep controls inline and wires a dedicated pause/resume control button', () => {
  ...
  assert.doesNotMatch(chatMessageChromeSource, /const handsFreeControlState = getHandsFreeComposerControlState\(handsFreeStatusPhase\);/);
  assert.match(sessionPresentationSource, /const handsFreeControlsRenderState = getChatComposerRuntimeHandsFreeControlsMobileRenderState\(\{/);
  ...
  assert.match(sessionPresentationSource, /handsFreeControls: \{[\s\S]*?isVisible: controlRenderState\.visibility\.handsFreeControls\.isVisible,[\s\S]*?status: handsFreeControlsRenderState\.status,[\s\S]*?controlState: handsFreeControlsRenderState\.controlState,[\s\S]*?onWake: onWakeHandsFree,[\s\S]*?onSleep: onSleepHandsFree,[\s\S]*?onResume: onResumeHandsFree,[\s\S]*?onPause: onPauseHandsFree,[\s\S]*?\.\.\.chrome\.handsFreeControls,/);
  assert.match(sessionPresentationSource, /const primaryOnPress = controlState\.primary\.action === "wake"\s+\? onWake\s+: onSleep/);
  assert.match(sessionPresentationSource, /const secondaryOnPress = controlState\.secondary\.action === "resume"\s+\? onResume\s+: onPause/);
  assert.match(sessionPresentationSource, /accessibilityRole: controlState\.primary\.accessibilityRole/);
  assert.match(sessionPresentationSource, /accessibilityLabel: controlState\.primary\.accessibilityLabel/);
  assert.match(sessionPresentationSource, /accessibilityRole: controlState\.secondary\.accessibilityRole/);
  assert.match(sessionPresentationSource, /accessibilityLabel: controlState\.secondary\.accessibilityLabel/);
  assert.match(sessionPresentationSource, /onPress: primaryOnPress/);
  assert.match(sessionPresentationSource, /text: controlState\.primary\.label/);
  assert.match(sessionPresentationSource, /onPress: secondaryOnPress/);
  assert.match(sessionPresentationSource, /text: controlState\.secondary\.label/);
  ...
  assert.match(sessionPresentationSource, /getChatComposerRuntimeControlMobileRenderState\(\{[\s\S]*?micPhase: composerControlMicPhase,[\s\S]*?listening: composerControlListening,[\s\S]*?colors: composerControlColors,[\s\S]*?\}\)/);
  ...
  assert.match(sessionPresentationSource, /onPressIn: controlRenderState\.visibility\.micButton\.shouldUsePushToTalk \? onMicPressIn : undefined/);
  assert.match(sessionPresentationSource, /onPress: controlRenderState\.visibility\.micButton\.shouldUseHandsFreePrimaryControl \? onMicPress : undefined/);
  assert.match(chatRuntimeMobileStylesSource, /handsFreeControlsRow:\s*\{\s+\.\.\.handsFreeStyleSlots\.controlsRow,\s+\}/);
  assert.match(chatRuntimeMobileStylesSource, /handsFreeControlButton:\s*\{\s+\.\.\.handsFreeStyleSlots\.controlButton,\s+\}/);
  assert.match(chatRuntimeMobileStylesSource, /handsFreeControlButtonText:\s*\{\s+\.\.\.handsFreeStyleSlots\.controlButtonText,\s+\}/);
  assert.match(sessionPresentationSource, /flexDirection:\s*surface\.controlsRow\.flexDirection/);
  assert.match(sessionPresentationSource, /borderColor:\s*colors\.controlButton\.borderColor/);
  assert.match(sessionPresentationSource, /backgroundColor:\s*colors\.controlButton\.backgroundColor/);
  assert.doesNotMatch(screenSource, /const mobileHandsFreeSurface = mobileHandsFreeSurfaceRenderState\.surface;/);
  assert.match(sessionPresentationSource, /const handsFreeSurface = getHandsFreeComposerMobileSurfaceRenderState\(\{\s+colors,\s+\}\)\.surface/);
  assert.match(sessionPresentationSource, /controlPressedOpacity: handsFreeSurface\.controlButton\.pressedOpacity/);
  assert.match(sessionPresentationSource, /activeOpacity: controlPressedOpacity/);
  assert.match(sessionPresentationSource, /color:\s*colors\.controlButtonText\.color/);
  assert.match(sessionPresentationSource, /fontWeight:\s*surface\.controlButtonText\.fontWeight/);
...
test('uses shared handsfree composer presentation helpers instead of local phase copy', () => {
  ...
  assert.match(chatRuntimeMobileStylesSource, /debugInfo:\s*\{\s+\.\.\.handsFreeStyleSlots\.debugPanel,/);
  assert.match(chatRuntimeMobileStylesSource, /debugText:\s*\{\s+\.\.\.handsFreeStyleSlots\.debugText,/);
  assert.match(sessionPresentationSource, /backgroundColor:\s*colors\.debugPanel\.backgroundColor/);
  assert.match(sessionPresentationSource, /borderLeftWidth:\s*surface\.debugPanel\.borderLeftWidth/);
  assert.match(sessionPresentationSource, /borderLeftColor:\s*colors\.debugPanel\.borderLeftColor/);
  assert.match(sessionPresentationSource, /color:\s*colors\.debugText\.color/);
  assert.match(sessionPresentationSource, /resolveChatRuntimeMobileFontFamily\(\s*surface\.debugText\.fontFamilyByPlatform,\s*platform \?\? "",\s*\)/);
  ...
  assert.doesNotMatch(chatMessageChromeSource, /formatHandsFreeSleepingDebugMessage/);
  assert.match(sessionPresentationSource, /formatHandsFreeSleepingDebugMessage/);
  ...
  assert.match(chatMessageChromeSource, /setDebugInfo\(formatChatComposerRuntimeHandsFreeSleepingDebugMessage\(wakePhrase\)\)/);
  assert.match(sessionPresentationSource, /export function formatChatComposerRuntimeHandsFreeSleepingDebugMessage/);
  ...
  assert.doesNotMatch(screenSource, /createHandsFreeComposerRecognizerErrorDebugState,/);
  assert.match(screenSource, /createChatComposerRuntimeHandsFreeRecognizerErrorDebugState,/);
  assert.match(screenSource, /setDebugInfo\(createChatComposerRuntimeHandsFreeRecognizerErrorDebugState\(message\)\.debugInfo\)/);
  ...
  assert.doesNotMatch(screenSource, /createHandsFreeComposerTranscriptAddedDebugState,/);
  assert.match(screenSource, /createChatComposerRuntimeHandsFreeTranscriptAddedDebugState,/);
  assert.match(screenSource, /setDebugInfo\(createChatComposerRuntimeHandsFreeTranscriptAddedDebugState\(\)\.debugInfo\)/);
  assert.doesNotMatch(screenSource, /createChatComposerHandsFreePermissionDeniedDebugState,/);
  assert.doesNotMatch(screenSource, /createHandsFreeComposerPermissionDeniedDebugState,/);
  assert.match(screenSource, /createChatComposerRuntimeHandsFreePermissionDeniedDebugState,/);
  assert.match(screenSource, /setDebugInfo\(createChatComposerRuntimeHandsFreePermissionDeniedDebugState\(\)\.debugInfo\)/);
  assert.doesNotMatch(screenSource, /from '@dotagents\/shared\/hands-free-controller'/);
  assert.match(sessionPresentationSource, /export function createChatComposerRuntimeHandsFreeTranscriptAddedDebugState/);
  assert.match(sessionPresentationSource, /export function createChatComposerRuntimeHandsFreePermissionDeniedDebugState/);
  assert.match(sessionPresentationSource, /export function createChatComposerRuntimeHandsFreeRecognizerErrorDebugState/);
  ...
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerDebugMessage\('enabled'\)/);
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerDebugMessage\('disabled'\)/);
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeHandsFreeDebugMessage\('enabled'\)/);
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeHandsFreeDebugMessage\('disabled'\)/);
  assert.match(sessionPresentationSource, /export function getChatComposerRuntimeHandsFreeDebugMessage/);
  ...
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerDebugMessage\('awake'\)/);
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerDebugMessage\('resumed'\)/);
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerDebugMessage\('paused'\)/);
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeHandsFreeDebugMessage\('awake'\)/);
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeHandsFreeDebugMessage\('resumed'\)/);
  assert.match(chatMessageChromeSource, /getChatComposerRuntimeHandsFreeDebugMessage\('paused'\)/);
  assert.doesNotMatch(chatMessageChromeSource, /from '@dotagents\/shared\/hands-free-controller'/);
  ...
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeComposerControlState/);
  assert.match(sessionPresentationSource, /getHandsFreeComposerControlState/);
  assert.doesNotMatch(screenSource, /getHandsFreeStatusSubtitle/);
  assert.doesNotMatch(chatMessageChromeSource, /getHandsFreeStatusSubtitle/);
  assert.match(sessionPresentationSource, /getHandsFreeStatusSubtitle/);
  ...
  assert.match(sessionPresentationSource, /getHandsFreeComposerPlaceholder/);
