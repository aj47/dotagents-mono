...
- Desktop modular config (`.agents`) path/source clarity in `apps/desktop/src/renderer/src/pages/settings-general.tsx` is a fresh settings UX follow-up, because the page already fetches global/workspace skills and memories layer data but currently only surfaces the top-level folder path.
