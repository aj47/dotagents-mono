import { REMOTE_SERVER_API_ROUTE_PATHS } from './remote-server-api';
import type {
...
export interface RemoteServerOperatorRouteRequestLike {
  body: unknown;
  params: unknown;
  query: unknown;
}

export interface RemoteServerOperatorRouteReplyLike {
  code(statusCode: number): { send(body?: unknown): unknown };
}

export type RemoteServerOperatorRouteResult = unknown | Promise<unknown>;

export type RemoteServerOperatorRouteHandler<
  Request extends RemoteServerOperatorRouteRequestLike,
  Reply extends RemoteServerOperatorRouteReplyLike,
> = (request: Request, reply: Reply) => RemoteServerOperatorRouteResult;

export interface RemoteServerOperatorRouteServer<
  Request extends RemoteServerOperatorRouteRequestLike = RemoteServerOperatorRouteRequestLike,
  Reply extends RemoteServerOperatorRouteReplyLike = RemoteServerOperatorRouteReplyLike,
> {
  get(path: string, handler: RemoteServerOperatorRouteHandler<Request, Reply>): unknown;
  post(path: string, handler: RemoteServerOperatorRouteHandler<Request, Reply>): unknown;
  patch(path: string, handler: RemoteServerOperatorRouteHandler<Request, Reply>): unknown;
  delete(path: string, handler: RemoteServerOperatorRouteHandler<Request, Reply>): unknown;
}
...
export function registerOperatorRoutes<
  ...
  Reply extends RemoteServerOperatorRouteReplyLike = RemoteServerOperatorRouteReplyLike,
>(
  fastify: RemoteServerOperatorRouteServer<Request, Reply>,
  options: OperatorRouteOptions<Request, Reply>,
): void {
  const {
    actions,
    providerSecretMask,
  ...
  fastify.get(API_ROUTES.operatorDiagnosticReport, async (_req, reply) => {
    const result = await actions.getOperatorDiagnosticReport();
    return reply.code(result.statusCode).send(result.body);
  });

  fastify.post(API_ROUTES.operatorDiagnosticReportSave, async (req, reply) => {
    const result = await actions.saveOperatorDiagnosticReport(req.body);
  ...
  fastify.post(API_ROUTES.operatorStopTtsPlayback, async (req, reply) => {
    const result = await actions.stopOperatorTtsPlayback();
    if (result.auditContext) {
      actions.setOperatorAuditContext(req, result.auditContext);
    }
    return reply.code(result.statusCode).send(result.body);
  ...
  fastify.post(API_ROUTES.operatorAgentSessionUnsnooze, async (req, reply) => {
    const params = req.params as { sessionId?: string };
    const result = await actions.unsnoozeOperatorAgentSession(params.sessionId);
    if (result.auditContext) {
      actions.setOperatorAuditContext(req, result.auditContext);
    }
    return reply.code(result.statusCode).send(result.body);
