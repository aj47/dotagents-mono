 ...
 *
 * Manages agent profiles (CRUD, migration, ACP integration).
 * Extracted to @dotagents/core — uses PathResolver instead of Electron's app.getPath.
 * Dependencies on builtin-tool-definitions and ACP registry are injected via setters.
 */
...
// ============================================================================
// Injectable dependencies — set by the host app (desktop, CLI, etc.)
// ============================================================================

/** Returns names of all builtin tools — injected by host app */
let _getBuiltinToolNames: () => string[] = () => []

/** ACP registry for syncing agent profiles — injected by host app */
let _acpRegistry: {
  registerAgent(definition: unknown): void
} | null = null

/**
 * Set the builtin tool names provider.
 * Must be called before using profile creation methods that need tool names.
 */
export function setBuiltinToolNamesProvider(fn: () => string[]): void {
  _getBuiltinToolNames = fn
}

/**
 * Set the ACP registry for agent profile syncing.
 */
export function setACPRegistry(registry: { registerAgent(definition: unknown): void }): void {
  _acpRegistry = registry
}

// ============================================================================
// Legacy types for migration — kept minimal, not exported
// ============================================================================

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Profile = Record<string, any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type ProfilesData = Record<string, any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Persona = Record<string, any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type PersonasData = Record<string, any>

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AgentProfilesData = Record<string, any>

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type MCPServerConfig = Record<string, any>

// ============================================================================
// Legacy conversion helpers (inline since they depend on desktop-only types)
// ============================================================================

function profileToAgentProfile(profile: Profile): AgentProfile {
  const now = Date.now()
  return {
    id: profile.id,
    name: profile.name,
...
function personaToAgentProfile(persona: Persona): AgentProfile {
  const now = Date.now()
  return {
    id: persona.id,
    name: persona.name,
...
function acpAgentConfigToAgentProfile(config: ACPAgentConfig): AgentProfile {
  const now = Date.now()
  return {
    id: `acp_${config.name}`,
    name: config.name,
...
// ============================================================================
// ACP agent definition type (minimal, for syncing)
// ============================================================================

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type ACPAgentDefinition = Record<string, any>

// ============================================================================
// Path resolution — lazy, uses PathResolver from service container
// ============================================================================
...
export function getAgentProfilesPath(): string {
  return path.join(getPathResolver().getUserDataPath(), "agent-profiles.json")
}

/**
 * Path to the agent profile conversations storage file.
 */
export function getAgentProfileConversationsPath(): string {
  return path.join(getPathResolver().getUserDataPath(), "agent-profile-conversations.json")
}

// Legacy paths for migration
function getLegacyProfilesPath(): string {
  return path.join(getPathResolver().getUserDataPath(), "profiles.json")
}

function getLegacyPersonasPath(): string {
  return path.join(getPathResolver().getUserDataPath(), "personas.json")
}
...
class AgentProfileService {
  ...
  private syncPromptsFromLayer(data: AgentProfilesData) {
    ...
    const mainAgent = data.profiles.find((p: any) => p.name === "main-agent")
    if (mainAgent) {
  ...
  private syncPromptsToLayer() {
    ...
    const mainAgent = this.profilesData.profiles.find((p: any) => p.name === "main-agent")
    if (!mainAgent) return
  ...
  private loadProfiles(): AgentProfilesData {
    ...
    if (globalResult.profiles.length > 0 || workspaceProfiles.length > 0) {
      ...
      try {
        if (fs.existsSync(getAgentProfilesPath())) {
          const legacyData = JSON.parse(fs.readFileSync(getAgentProfilesPath(), "utf8")) as AgentProfilesData
    ...
    try {
      if (fs.existsSync(getAgentProfilesPath())) {
        const data = JSON.parse(fs.readFileSync(getAgentProfilesPath(), "utf8")) as AgentProfilesData
  ...
  private migrateFromLegacy(): AgentProfile[] {
    ...
    try {
      if (fs.existsSync(getLegacyProfilesPath())) {
        const data = JSON.parse(fs.readFileSync(getLegacyProfilesPath(), "utf8")) as ProfilesData
    ...
    try {
      if (fs.existsSync(getLegacyPersonasPath())) {
        const data = JSON.parse(fs.readFileSync(getLegacyPersonasPath(), "utf8")) as PersonasData
  ...
  private saveProfiles(): void {
    ...
    try {
      ...
      fs.writeFileSync(getAgentProfilesPath(), JSON.stringify(this.profilesData, null, 2))
    } catch (error) {
  ...
  private loadConversations(): void {
    try {
      if (fs.existsSync(getAgentProfileConversationsPath())) {
        this.conversationsData = JSON.parse(
          fs.readFileSync(getAgentProfileConversationsPath(), "utf8")
        )
      }
    } catch (error) {
  ...
  getById(id: string): AgentProfile | undefined {
    return this.profilesData?.profiles.find((p: any) => p.id === id)
  }
  ...
  getByName(name: string): AgentProfile | undefined {
    ...
    return this.profilesData?.profiles.find((p: any) => p.name === name)
      || this.profilesData?.profiles.find((p: any) => p.displayName === name)
  }
  ...
  delete(id: string): boolean {
    ...
    const index = this.profilesData.profiles.findIndex((p: any) => p.id === id)
  ...
  syncAgentProfilesToACPRegistry(): void {
    ...
    for (const profile of enabledTargets) {
      ...
      _acpRegistry?.registerAgent(definition)
    }
  ...
  importProfile(profileJson: string): AgentProfile {
    try {
      ...
      const builtinToolNames = _getBuiltinToolNames()
