...
// Injected PathResolver for platform-agnostic path resolution
let pathResolver: PathResolver | null = null

/**
 * Set the PathResolver for model storage paths.
 * Must be called before any TTS operations.
 */
export function setSupertonicTTSPathResolver(resolver: PathResolver): void {
  pathResolver = resolver
}

function requirePathResolver(): PathResolver {
  if (!pathResolver) {
    throw new Error("[Supertonic] PathResolver not set. Call setSupertonicTTSPathResolver() before using TTS.")
  }
  return pathResolver
}
...
function getModelsPath(): string {
  return path.join(requirePathResolver().getUserDataPath(), "models", "supertonic")
}
