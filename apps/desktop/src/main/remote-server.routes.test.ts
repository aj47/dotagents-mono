...
describe("remote-server route registration", () => {
  ...
  it("exposes remote server, tunnel, and Discord settings in the remote settings GET/PATCH routes", () => {
    ...
    expect(settingsActionsSource).toContain("buildSettingsResponse(cfg, {")
    expect(settingsActionsSource).toContain("remoteServerApiKey: getMaskedRemoteServerApiKey(cfg.remoteServerApiKey)")
    expect(settingsActionsSource).toContain("discordBotToken: getMaskedDiscordBotToken(cfg)")
    expect(settingsActionsSource).toContain("discordDefaultProfileId: getDiscordResolvedDefaultProfileId(cfg).profileId ?? \"\"")
    expect(settingsActionsSource).toContain("acpxAgents: agentProfileService.getAll()")
    ...
    expect(settingsActionsSource).toContain("const requestBody = getSettingsUpdateRequestRecord(body)")
    expect(settingsActionsSource).toContain("buildSettingsUpdatePatch(requestBody, cfg, masks)")
    expect(settingsActionsSource).toContain("const remoteServerLifecycleAction = getRemoteServerLifecycleAction(cfg, nextConfig)")
    expect(settingsActionsSource).toContain("getDiscordLifecycleAction(cfg, nextConfig)")
    expect(settingsActionsSource).toContain("await applyDiscordLifecycleAction(discordLifecycleAction)")
  })
  ...
  it("delegates agent profile route behavior to agent profile actions", () => {
    ...
    expect(agentProfileActionsSource).toContain("filterAgentProfilesByRole(profiles, role)")
    expect(agentProfileActionsSource).toContain("buildAgentProfilesResponse(profiles)")
    expect(agentProfileActionsSource).toContain("buildAgentProfileToggleResponse(id ?? \"\", updated?.enabled ?? !profile.enabled)")
    expect(agentProfileActionsSource).toContain("buildAgentProfileDetailResponse(profile)")
    expect(agentProfileActionsSource).toContain("parseAgentProfileCreateRequestBody(body)")
    expect(agentProfileActionsSource).toContain("parseAgentProfileUpdateRequestBody(body")
    expect(agentProfileActionsSource).toContain("buildAgentProfileMutationDetailResponse(updatedProfile)")
    expect(agentProfileActionsSource).toContain("buildAgentProfileDeleteResponse()")
  })
  ...
  it("keeps operator summaries redacted and restart actions scheduled", () => {
    ...
    expect(operatorObservabilityActionsSource).toContain("buildOperatorHealthSnapshot")
    expect(operatorObservabilityActionsSource).toContain("buildOperatorRecentErrorsResponse")
    expect(operatorObservabilityActionsSource).toContain("buildOperatorLogsResponse")
    ...
    expect(operatorObservabilityActionsSource).toContain("buildOperatorConversationsResponse(history, count)")
    expect(operatorObservabilityActionsSource).toContain("normalizeOperatorLogLevel(level)")
    expect(operatorObservabilityActionsSource).toContain("clampOperatorCount(count")
    ...
    expect(operatorTunnelActionsSource).toContain("getConfiguredCloudflareTunnelStartPlan")
    ...
    expect(operatorRestartActionsSource).toContain("buildOperatorRestartRemoteServerActionResponse(isRunning)")
    expect(operatorRestartActionsSource).toContain("buildOperatorRestartAppActionResponse(appVersion)")
    expect(operatorRestartActionsSource).toContain("buildOperatorActionAuditContext(response)")
    ...
    expect(operatorApiKeyActionsSource).toContain("buildOperatorApiKeyRotationResponse(apiKey)")
    expect(operatorApiKeyActionsSource).toContain("buildOperatorApiKeyRotationAuditContext()")
    expect(operatorApiKeyActionsSource).toContain("buildOperatorApiKeyRotationFailureAuditContext()")
    // Runtime status shaping stays shared while desktop supplies process and service state.
    expect(operatorObservabilityActionsSource).toContain("buildOperatorRuntimeStatus({")
    expect(operatorObservabilityActionsSource).toContain("system: {")
    ...
    expect(operatorObservabilityActionsSource).toContain("activeSessions: agentSessionTracker.getActiveSessions()")
    expect(operatorObservabilityActionsSource).toContain("recentSessions: agentSessionTracker.getRecentSessions(")
    // Conversations endpoint
    expect(operatorObservabilityActionsSource).toContain("buildOperatorConversationsResponse(history, count)")
    expect(operatorObservabilityActionsSource).toContain("conversationService.getConversationHistory()")
    ...
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetsResponse")
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetMutationResponse")
    expect(source).toContain("PROVIDER_SECRET_MASK")
    expect(operatorModelPresetActionsSource).toContain("getModelPresetActivationUpdates")
    expect(operatorModelPresetActionsSource).toContain("upsertModelPresetOverride")
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetCreateAuditContext")
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetUpdateAuditContext")
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetDeleteAuditContext")
    expect(operatorModelPresetActionsSource).toContain("buildModelPresetMutationFailureAuditContext")
  })
  ...
  it("audits sensitive settings updates without persisting secrets", () => {
    ...
    expect(settingsPatchSection).toContain("actions.recordOperatorAuditEvent(req, result.auditContext)")
    expect(settingsActionsSource).toContain("let attemptedSensitiveSettingsKeys: string[] = []")
    expect(settingsActionsSource).toContain("getSensitiveOperatorSettingsKeys(requestBody)")
    expect(settingsActionsSource).toContain("const sensitiveUpdatedKeys = getSensitiveOperatorSettingsKeys(updates)")
    expect(settingsActionsSource).toContain("buildSettingsSensitiveNoValidUpdateAuditContext(attemptedSensitiveSettingsKeys)")
    expect(settingsActionsSource).toContain("buildSettingsSensitiveUpdateAuditContext(sensitiveUpdatedKeys")
    expect(settingsActionsSource).toContain("buildSettingsSensitiveUpdateFailureAuditContext(attemptedSensitiveSettingsKeys)")
    expect(settingsActionsSource).toContain("buildSettingsUpdateResponse(updates)")
    expect(settingsActionsSource).toContain("remoteServerLifecycleAction")
    expect(settingsActionsSource).toContain("discordLifecycleAction")
