...
describe("settings general controlled config bindings", () => {
  ...
  it("shows visible feedback if disabling desktop TTS cannot stop speech in other windows", () => {
    expect(settingsGeneralSource).toContain('console.error("Failed to stop TTS in all windows:", error)')
    expect(settingsGeneralSource).toContain('await tipcClient.stopAllTts()')
    expect(settingsGeneralSource).toContain('Disabled TTS for this window, but failed to stop speech in other windows.')
    expect(settingsGeneralSource).toContain('getActionErrorMessage(error, "Please retry if audio is still playing.")')
  })
})