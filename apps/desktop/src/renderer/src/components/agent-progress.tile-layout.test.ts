...
describe("agent progress tile layout", () => {
  ...
  it("surfaces latest delegated activity and a richer live details dialog from the tile chat area", () => {
    ...
    expect(agentProgressSource).toContain('aria-label={isCollapsed ? "Expand delegations" : "Collapse delegations"}')
  ...
  it("stops delegated tool rows from showing a loading spinner after terminal completion", () => {
    ...
    expect(agentProgressSource).toContain('function isStructuredToolInvocationMessage(message: ACPSubAgentMessage): boolean {')
    expect(agentProgressSource).toContain('if (isStructuredToolInvocationMessage(message) || isDelegatedToolResultMessage(message)) {')
    expect(agentProgressSource).toContain('const isToolUseMessage = !!parsedUseMessage || isStructuredToolUseMessage')
