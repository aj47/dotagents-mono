...
- **Headless CLI agent session controls** now also expose `/status`, `/sessions`, `/session-stop <session-id-or-prefix>`, and `/sessions-clear`, and those commands share the same tracked-session snapshot plus per-session stop/cleanup helper path that the desktop Sessions page, active-session sidebar, and stop buttons use through `tipc.ts`.
- **Headless CLI message queue controls** now also expose `/queues`, `/queue [conversation-id-or-prefix]`, `/queue-edit <message-id-or-prefix> <text>`, `/queue-remove <message-id-or-prefix> [conversation-id-or-prefix]`, `/queue-retry <message-id-or-prefix> [conversation-id-or-prefix]`, `/queue-clear [conversation-id-or-prefix]`, `/queue-pause [conversation-id-or-prefix]`, and `/queue-resume [conversation-id-or-prefix]`, and those commands share the same queue snapshot, queued-message selection, retry/edit recovery, pause/resume, and queued follow-up replay helper path that the desktop message-queue panel uses through `tipc.ts`.
...
- **Headless CLI MCP tool controls** now also expose `/mcp-tools`, `/mcp-tool-show <tool-name-prefix>`, `/mcp-tool-enable <tool-name-prefix>`, `/mcp-tool-disable <tool-name-prefix>`, `/mcp-source-enable <source-name-prefix>`, and `/mcp-source-disable <source-name-prefix>`, and those commands share the same MCP/runtime tool summary, selection, and source-wide toggle helpers that the desktop capabilities UI uses through `tipc.ts`.
- **Headless CLI MCP OAuth controls** now also expose `/mcp-oauth`, `/mcp-oauth-show <server-name-prefix>`, `/mcp-oauth-start <server-name-prefix>`, `/mcp-oauth-complete <server-name-prefix> <code> <state>`, and `/mcp-oauth-revoke <server-name-prefix>`, and those commands share the same streamable HTTP server filtering, auth-status inspection, headless-safe flow initiation, manual code/state completion, and token revocation helpers that the desktop capabilities UI uses through `tipc.ts`.
- **Headless CLI WhatsApp controls** now also expose `/whatsapp-status`, `/whatsapp-connect`, `/whatsapp-disconnect`, and `/whatsapp-logout`, and those commands share the same WhatsApp MCP availability checks, status parsing, QR-required connection handling, and disconnect/logout helpers that the desktop WhatsApp settings screen uses through `tipc.ts`.
- **Headless CLI agent selection** now also exposes `/status`, `/agents`, and `/agent <agent-id-or-name>`, and those current-agent reads plus agent switches now share one managed current-profile helper with desktop profile pickers and remote/mobile profile switching so disabled selections are rejected consistently before profile-specific model/audio and MCP runtime config apply.
...
- **Desktop legacy profile IPC handlers** now also share managed profile helpers, so backward-compatible `getProfile*`, `createProfile(...)`, `updateProfile(...)`, `deleteProfile(...)`, and `setCurrentProfile(...)` calls reuse the same current-profile guard and agent-profile validation/persistence path before reshaping results into the legacy payload format.
...
- **Headless CLI knowledge note controls** now also expose `/notes`, `/note-show <note-id-or-prefix>`, `/note-search <query>`, `/note-new <json>`, `/note-edit <note-id-or-prefix> <json>`, `/note-delete <note-id-or-prefix>`, `/note-delete-many <json-array-of-note-ids-or-prefixes>`, and `/note-delete-all`, and those commands share the same note list/search/save/update/delete helpers that the desktop knowledge workspace and remote note API use.
...
- **Headless CLI settings controls** now also expose `/settings` and `/settings-edit <json>`, and those commands share the same normalized settings snapshot plus validated patch helper that the remote `/v1/settings` API uses, including shared ACP main-agent options and remote-access defaults.
- **Headless CLI model catalog controls** now also expose `/models <provider-id>`, `/models-preset <json>`, `/model-info <model-id> [provider-id]`, `/models-dev`, and `/models-refresh`, and those commands share the same provider-model fetch, preset-scoped fetch, and models.dev lookup/refresh helper path that the desktop model selectors use through `tipc.ts`, while the remote `/v1/models/:providerId` route now reuses that same helper for provider catalog reads.
- **Headless CLI local provider controls** now also expose `/parakeet-status`, `/parakeet-download`, `/kitten-status`, `/kitten-download`, `/supertonic-status`, and `/supertonic-download`, and those commands share the same local-model status and download helpers that the desktop Providers settings page uses through `tipc.ts`.
- **Headless CLI speech generation** now also exposes `/tts <json>`, `/kitten-speak <json>`, and `/supertonic-speak <json>`, and those commands share the same speech helper path that desktop response narration plus the Providers page voice-preview buttons use through `tipc.ts`, so preprocessing, provider defaults, and local WAV conversion stay aligned before the CLI writes audio files or the desktop UI plays them.
- **Headless CLI remote access controls** now also expose `/remote-status`, `/remote-qr`, `/cloudflare-status`, `/cloudflare-start`, `/cloudflare-stop`, and `/cloudflare-list`, and those commands share the same remote-server status, manual QR printing, Cloudflare install/login checks, tunnel listing, and tunnel lifecycle helpers that the desktop `Settings > Remote Server` page uses through `tipc.ts`.
- **Headless CLI bundle management** now also exposes `/bundle-items`, `/bundle-export <path> [json]`, `/bundle-preview <path>`, `/bundle-import <path> [json]`, and `/bundle-publish-payload <json>`, and those commands share the same layered bundle selection, merged conflict preview, publish payload generation, and post-import runtime refresh helpers that the desktop bundle dialogs and sandbox import/restore flows use through `tipc.ts`.
- **Headless CLI sandbox slot management** now also exposes `/sandboxes`, `/sandbox-baseline-save`, `/sandbox-baseline-restore`, `/sandbox-slot-save <name>`, `/sandbox-slot-switch <name>`, `/sandbox-slot-delete <name>`, `/sandbox-slot-rename <old> <new>`, and `/sandbox-bundle-import <path> <slot-name> [json]`, and those commands share the same baseline save/restore, slot selection, switch/delete/rename, and bundle-to-slot import helpers that the desktop sandbox slot switcher uses through `tipc.ts`.
...
- **Desktop settings saves plus headless/remote settings updates** now also share one config-persistence helper, so model-cache invalidation, remote-access reconciliation, WhatsApp MCP lifecycle, Langfuse reinitialization, and MCP profile cleanup run the same way regardless of whether the change starts in the desktop UI, headless CLI, or remote/mobile client.
...
- **MCP tool management** now also shares one main-process helper, so headless MCP tool commands plus the desktop capabilities UI tool toggles all resolve exact-name/unique-prefix tool selection, source selection, and per-tool or source-wide enable/disable behavior the same way.
- **MCP OAuth management** now also shares one main-process helper, so headless OAuth commands plus the desktop capabilities UI all resolve streamable HTTP server selection, auth-status reads, headless-safe flow initiation, manual code/state completion, and token revocation the same way.
- **WhatsApp status/connect/disconnect/logout** now also shares one main-process helper, so headless WhatsApp commands and the desktop WhatsApp settings page interpret MCP tool responses, QR-required states, and failure messages the same way before presenting them in the terminal or GUI.
- **Speech generation and local voice previews** now also share one main-process helper, so headless audio-export commands, desktop response narration, and the Providers page preview buttons all reuse the same preprocessing, provider/voice defaults, and local WAV conversion logic before output diverges into saved files or playback.
